...
app.post('/api/ai/search', async (req, res) => {
  ...
  const tinyFishKey = process.env.TINYFISH_API_KEY || process.env.VITE_TINYFISH_API_KEY || '';
  
  // 1. LOG: TinyFish request sent
  console.log(`[AI Server] TinyFish request sent. Query: "${query}", Key present: ${!!tinyFishKey}`);
  ...
  try {
    ...
    if (tinyFishKey) {
      try {
        ...
        if (response.ok) {
          ...
          success = true;
          // 2. LOG: TinyFish response received
          console.log(`[AI Server] TinyFish response received. Status: ${response.status}, success: true`);
        } else {
          console.warn(`[AI Server] TinyFish primary endpoint failed with status: ${response.status}`);
          const fallbackRes = await fetch(`https://api.tinyfish.ai/v1/web/search?query=${encodeURIComponent(query)}&limit=10`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${tinyFishKey}`
            }
          ...
          if (fallbackRes.ok) {
            ...
            success = true;
            console.log(`[AI Server] TinyFish fallback response received. Status: ${fallbackRes.status}, success: true`);
          }
        }
      } catch (err: any) {
        console.error('[AI Server] Error fetching TinyFish API:', err.message);
      }
    }

    // Process raw TinyFish search results
    let rawResults: any[] = [];
    ...
    if (rawResults.length > 0) {
      searchResultsList = rawResults.map((item: any) => {
        const title = item.title || 'Educational Resource';
        const uri = item.url || item.uri || item.link || `https://en.wikipedia.org/wiki/${encodeURIComponent(query)}`;
        const description = item.snippet || item.description || item.content || 'Real-time learning material and online documentation.';
        
        let published_source = item.source || item.published_source || '';
        if (!published_source && uri) {
          try {
      ...
      console.log(`[AI Server] TinyFish search results mapped. Total items: ${searchResultsList.length}`);
    } else {
      // Roll over to live web search using native search grounding (real results, not synthesized)
      console.log(`[AI Server] TinyFish returned empty or inactive. Activating live search grounding tool...`);
      ...
      if (searchAi) {
        const response = await searchAi.models.generateContent({
          ...
          contents: `Return actual real-time search results for the topic: "${query}". Do not make up results.`,
        ...
        if (webChunks.length > 0) {
          searchResultsList = webChunks.map((item: any) => {
            ...
            try {
              ...
              published_source = 'Google Search';
            }
            return {
              title: item.title || 'Live Search Result',
              description: item.title || 'Grounded live search discovery.',
              uri: uri,
    ...
    // If still no results, fallback to active scholarly links
    if (searchResultsList.length === 0) {
      searchResultsList = [
        { title: `${query} on Wikipedia`, description: `Comprehensive academic article covering "${query}".`, uri: `https://en.wikipedia.org/wiki/${encodeURIComponent(query)}`, published_source: 'wikipedia.org' },
        { title: `${query} on Khan Academy`, description: "Free educational lessons, video courses, and practices.", uri: "https://www.khanacademy.org", published_source: 'khanacademy.org' },
        { title: `${query} on Google Scholar`, description: "Scholarly research papers and documents repository.", uri: `https://scholar.google.com/scholar?q=${encodeURIComponent(query)}`, published_source: 'scholar.google.com' }
      ];
    }
    ...
    const summarizerAi = ai || (process.env.GEMINI_API_KEY ? new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY }) : null);
    if (summarizerAi) {
    ...
    // 3. LOG: Search results rendered
    console.log(`[AI Server] Search results rendered for query: "${query}". Found ${searchResultsList.length} references.`);

  } catch (err: any) {
    console.error('[AI Server] Search execution failure:', err);
    summaryText = `An error occurred while executing the search: ${err.message}`;
  }
