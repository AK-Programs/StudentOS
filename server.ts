...
app.post('/api/ai/search', async (req, res) => {
  ...
  // 1. Try Native Gemini Search Grounding
  if (ai) {
    try {
      console.log(`[AI Server] Performing Google Search Grounding for: "${query}"`);
      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: `Provide a highly detailed, clean academic summary and research findings about: "${query}". Include Key Findings, Bulleted facts, and list trusted web sources with titles.`,
        config: {
          tools: [{ googleSearch: {} }],
        },
      });
      ...
      console.warn('[AI Server] Native Gemini Search Grounding failed, falling back...', err);
    }
  }

  // 2. Try OpenRouter Search Simulation with Web Links
  const openRouterKey = process.env.OPENROUTER_API_KEY;
  if (openRouterKey) {
    try {
      console.log(`[AI Server] Performing OpenRouter Search Simulation for: "${query}"`);
      const systemInstruction = `You are Orion, an elite academic research assistant with real-time simulated web search capabilities.
      Generate a comprehensive, high-yield search report for the query: "${query}".
      Structure your response with clear headings (e.g., Key Findings, Detailed Analysis).
      Crucially, list 3 to 4 extremely relevant, high-quality real web sources (like Wikipedia, NASA, Khan Academy, Britannica) with their full URLs.
      Format the sources in markdown: [Source Title](URL).`;
      ...
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: 'system', content: systemInstruction },
            { role: 'user', content: `Search the web and research: "${query}"` }
          ],
          temperature: 0.7,
        })
      });
      ...
      if (response.ok) {
        ...
        // Extract markdown links from text to populate sources structure
      ...
      console.warn('[AI Server] OpenRouter Search Simulation failed:', err);
    }
  }

  // 3. Complete Offline Elegant Fallback (No Keys)
  const lowercaseQuery = query.toLowerCase();
  let text = `### 📡 Orion Research Search Results: "${query}"\n\n`;
  let sources: { title: string; uri: string }[] = [];

  if (lowercaseQuery.includes('newton') || lowercaseQuery.includes('force') || lowercaseQuery.includes('physics')) {
    text += `**Newton's Laws of Motion** describe the relationship between the motion of an object and the forces acting on it.\n\n` +
      ...
      `* **Second Law ($F = ma$)**: The acceleration of an object is dependent upon two variables - the net force acting upon the object and the mass of the object.\n` +
      `* **Third Law (Action-Reaction)**: For every action, there is an equal and opposite reaction.\n\n` +
      `### Key Discovery Findings:\n` +
      `- Centripetal force is a center-seeking force that keeps an object in circular motion. The work done is zero because force is perpendicular to displacement.\n` +
      `- Classic demonstrations include pendulums, planetary orbits, and vehicular turns on banked roads.`;
    sources = [
      { title: "Wikipedia: Newton's Laws of Motion", uri: "https://en.wikipedia.org/wiki/Newton%27s_laws_of_motion" },
      { title: "Khan Academy: Physics - Newton's Laws", uri: "https://www.khanacademy.org/science/physics/forces-newtons-laws" },
      { title: "NASA: Newton's Laws of Motion", uri: "https://www.grc.nasa.gov/www/k-12/airplane/newton.html" }
    ];
  } else if (lowercaseQuery.includes('cell') || lowercaseQuery.includes('respiration') || lowercaseQuery.includes('biology')) {
    text += `**Cellular Respiration** is the process by which biological cells convert chemical energy from nutrients into adenosine triphosphate (ATP).\n\n` +
      `* **Glycolysis**: The breakdown of glucose into pyruvate, yielding 2 ATP and 2 NADH molecules.\n` +
      `* **Citric Acid Cycle (Krebs)**: Occurs in the mitochondrial matrix, producing electron carriers (NADH and FADH2) and 2 ATP.\n` +
      `* **Oxidative Phosphorylation**: Generates the bulk of ATP (around 28-32 ATP) via the electron transport chain.`;
    sources = [
      { title: "Wikipedia: Cellular Respiration", uri: "https://en.wikipedia.org/wiki/Cellular_respiration" },
      { title: "Khan Academy: Biology - Cellular Respiration", uri: "https://www.khanacademy.org/science/ap-biology/cellular-energetics/cellular-respiration-ap/v/introduction-to-cellular-respiration" }
    ];
  } else {
    text += `Here are the academic research findings for **"${query}"**:\n\n` +
      `1. **Core Concept**: Analyzing ${query} provides significant insights into modern educational syllabi.\n` +
      `2. **Methodology**: Spaced retrieval practice and deep analogies help students synthesize this topic rapidly.\n` +
      `3. **Key Fact**: Establishing clean, baseline definitions ensures structured comprehension.\n\n` +
      `*Note: Active real-time web search can be fully unlocked by adding a GEMINI_API_KEY inside the Secrets panel.*`;
    sources = [
      { title: "Wikipedia: Search Topic", uri: `https://en.wikipedia.org/wiki/Special:Search?search=${encodeURIComponent(query)}` },
      { title: "Google Scholar: Search Results", uri: `https://scholar.google.com/scholar?q=${encodeURIComponent(query)}` }
    ];
  }
...
app.post('/api/ai/generate-notes', async (req, res) => {
  ...
  const systemInstruction = `You are an academic scholar and content copywriter. 
  Your job is to generate extremely polished, dense, and well-structured lecture and study notes in standard Markdown.
  You MUST format the output meticulously using Markdown:
  - Use elegant headings like # title, ## main sections, ### subheadings
  - Employ list elements, checklists, quotes or highlights, and bold terms where key terminology is first introduced.
  - If mathematical equations or algorithms are involved, use clean formulas, code blocks, or standard math patterns.
  - Always end with a clean "📚 Active Recall Quiz" containing 3 challenge questions to test student comprehension.
  
  Format requested: ${format || 'Detailed Lecture Notes'}.`;

  const userPrompt = `Generate a high-yield study sheet about the topic: "${topic}".
  Additional user notes/intentions: "${customPrompt || 'None provided'}"
  Ensure the formatting is rich, deeply engaging, and conforms to high-contrast scholastic typography.`;
  ...
  try {
    ...
    if (!openRouterKey && !geminiKey) {
      // Simulated mock content for offline fallback
      const mockContent = `# Study Sheet: ${topic}
      
## 1. Executive Concept Map
*This is an elegant simulated blueprint of ${topic} since no active API key is configured. Go to Secrets (Settings menu) to activate full AI.*

- **Core Concept**: ${topic} serves as a foundation for scholastic development and modern curricula.
- **Key Equation/Principle**: $E = mc^2$ or equivalent structural models can visualize key properties.
- **Practical Application**: Designing workflows, optimizing memory grids, or passing standard certification pathways.

## 2. Advanced Analysis & Analogy
Imagine ${topic} like an orchestra. Each instrument (sub-component) must tune to the same frequency. If one section falls out of rhythmic sync (improper initialization), the entire performance yields unexpected latency (runtime crash).

## 📚 Active Recall Quiz
1. What is the primary functional constraint of ${topic}?
2. How can you demonstrate the balance theorem in practical test applications?
3. Design a scaffold example using standard campus presets.`;

      return res.json({ text: mockContent });
    }
