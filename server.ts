...
app.post('/api/ai/search', async (req, res) => {
  ...
  console.log(`[AI Server] Tavily request sent. Query: "${query}", Key present: ${!!tavilyKey}`);
  ...
  try {
    ...
    if (tavilyKey) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`[AI Server] Tavily response received. Status: ${response.status}, success: true`);
        } else {
          console.warn(`[AI Server] Tavily endpoint failed with status: ${response.status}`);
        }
      } catch (err) {
        console.error('[AI Server] Error fetching Tavily API:', err.message);
      }
    }
    ...
    if (rawResults.length > 0) {
      searchResultsList = rawResults.map((item) => {
        ...
        const uri = item.url || `https://en.wikipedia.org/wiki/${encodeURIComponent(query)}`;
      ...
      console.log(`[AI Server] Tavily search results mapped. Total items: ${searchResultsList.length}`);
    } else {
      console.log(`[AI Server] Tavily returned empty or inactive. Activating live search grounding tool...`);
      ...
      if (searchAi) {
        ...
        if (webChunks.length > 0) {
          searchResultsList = webChunks.map((item) => {
            const uri = item.uri;
            let published_source = '';
            try {
              published_source = new URL(uri).hostname.replace('www.', '');
