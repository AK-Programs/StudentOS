...
app.post('/api/ai/mermaid', async (req, res) => {
  ...
  try {
    ...
    if (aiGen) {
      const prompt = `
You are an expert Mermaid.js diagram author for educational concepts.
Generate valid Mermaid code syntax for topic: "${query}".

Determined type based on topic:
- Flowchart (graph TD or graph LR)
- Mindmap (mindmap)
- Sequence diagram (sequenceDiagram)
- Timeline (timeline)
- Class or State diagram (classDiagram)

Formatting rules:
- Return ONLY the raw Mermaid code block string.
- Do NOT include markdown code block ticks like \`\`\`mermaid.
- Keep node labels clear and concise for educational whiteboards.
`;
      ...
      if (code) {
        return res.json({ code, title: query });
      }
    }

    // Fallback Mermaid code for offline mode
    const qLower = query.toLowerCase();
    let fallbackCode = `graph TD\n  Start[${query}] --> Concept1[Core Theory]\n  Concept1 --> Sub1[Key Principles]\n  Concept1 --> Sub2[Practical Applications]\n  Sub1 --> End[Conclusion]`;
    if (qLower.includes('mind') || qLower.includes('map') || qLower.includes('photosynthesis')) {
      fallbackCode = `mindmap\n  root((Photosynthesis))\n    Light Reactions\n      Sunlight Absorption\n      Water Splitting\n      Oxygen Release\n    Calvin Cycle\n      CO2 Fixation\n      Sugar Synthesis`;
    } else if (qLower.includes('water') || qLower.includes('cycle')) {
      fallbackCode = `graph TD\n  Ocean[Ocean Water] -->|Evaporation| Vapor[Atmospheric Vapor]\n  Trees[Forests] -->|Transpiration| Vapor\n  Vapor -->|Condensation| Clouds[Clouds]\n  Clouds -->|Precipitation| Rain[Rain / Snow]\n  Rain -->|Runoff & Infiltration| Ocean`;
    } else if (qLower.includes('timeline') || qLower.includes('history')) {
      fallbackCode = `timeline\n  title Process Timeline\n  Phase 1 : Discovery & Research\n  Phase 2 : Design & Prototyping\n  Phase 3 : Testing & Launch`;
    }

    return res.json({ code: fallbackCode, title: query });
  } catch (err: any) {
...
app.post('/api/ai/svg-diagram', async (req, res) => {
  ...
  try {
    ...
    if (aiGen) {
      const prompt = `
You are an expert educational illustrator generating raw inline SVG diagrams for whiteboards.
Generate a valid, visually beautiful SVG diagram for subject "${subject}" topic: "${query}".

Guidelines:
- Width: 600, Height: 450, viewBox="0 0 600 450"
- Style: Modern dark mode classroom palette (background: #0f172a or transparent, strokes: #818cf8, #34d399, #f59e0b, #ec4899, text: #f8fafc)
- Include key labels with clear typography (<text> tags).
- Include distinct visual components with rounded rectangles, circles, or path lines.
- Return ONLY the raw <svg>...</svg> element text. Do not wrap in markdown or backticks.
`;
      const response = await aiGen.models.generateContent({
        model: 'gemini-3.5-flash',
        contents: prompt,
      ...
      if (svg.includes('<svg')) {
        return res.json({ svg, title: query, subject });
      }
    }

    // Educational SVG presets for offline fallback
    let fallbackSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 400" width="600" height="400">
      <rect width="600" height="400" rx="20" fill="#0f172a" stroke="#334155" stroke-width="2"/>
      <text x="300" y="40" fill="#818cf8" font-size="20" font-weight="bold" text-anchor="middle">${query.toUpperCase()}</text>
      <circle cx="200" cy="200" r="70" fill="#312e81" stroke="#6366f1" stroke-width="3"/>
      <text x="200" y="205" fill="#ffffff" font-size="14" text-anchor="middle">Core Engine</text>
      <rect x="350" y="150" width="160" height="100" rx="12" fill="#064e3b" stroke="#10b981" stroke-width="3"/>
      <text x="430" y="205" fill="#ffffff" font-size="14" text-anchor="middle">Output Unit</text>
      <path d="M 270 200 L 350 200" stroke="#f59e0b" stroke-width="4" marker-end="url(#arrow)"/>
      <text x="310" y="190" fill="#f59e0b" font-size="12" text-anchor="middle">Flow</text>
    </svg>`;

    return res.json({ svg: fallbackSvg, title: query, subject });
  } catch (err: any) {
