...
app.post('/api/ai/search', async (req, res) => {
  ...
  try {
    console.log(`[AI Server] Performing Native Gemini Search for: "${query}"`);
    ...
    console.warn('[AI Server] Gemini Search API request failed:', err);
  }

  if (searchResultsList.length === 0) {
     // Fallback to text generation if search tools failed
     try {
       const aiResponse = await generateAICompletion(
         "You are Orion Search. Always return a valid JSON.",
         `Create a mock search result JSON for: "${query}". Format: {"text": "summary...", "sources": [{"title": "title", "description": "desc", "uri": "https://example.com"}]}`
       );
       const jsonStr = aiResponse.substring(aiResponse.indexOf('{'), aiResponse.lastIndexOf('}') + 1);
       const parsed = JSON.parse(jsonStr);
       summaryText = parsed.text;
       searchResultsList = parsed.sources || [];
     } catch (e) {
       summaryText = "Could not find reliable information for this query.";
     }
  }
