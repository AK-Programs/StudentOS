...
// Safety check - prevent blank screen if keys are missing
if (!resolvedApiKey || !resolvedProjectId) {
  console.error(
    "❌ Firebase configuration is missing!\n" +
    "Please set these environment variables on Vercel:\n" +
...
console.log("Project ID:", resolvedProjectId);
console.log("Auth Domain:", resolvedAuthDomain);
console.log("Database ID:", resolvedFirestoreDatabaseId);
if (typeof window !== 'undefined') {
  console.log("Exact Deployed URL:", window.location.href);
  console.log("Hostname:", window.location.hostname);
  
  if (resolvedProjectId === "gen-lang-client-0785563242") {
    console.error(
      "❌ [Firebase Security Alert]\n" +
      "You are running on a custom production domain but still using the sandbox Firebase project ID: gen-lang-client-0785563242.\n" +
      "Google Sign-In will fail with auth/unauthorized-domain because you cannot add custom domains to the sandbox project.\n" +
      "To fix:\n" +
      "1. Go to your Vercel dashboard -> Settings -> Environment variables.\n" +
      "2. Add your custom Firebase credentials:\n" +
      "   - VITE_FIREBASE_API_KEY\n" +
      "   - VITE_FIREBASE_AUTH_DOMAIN\n" +
      "   - VITE_FIREBASE_PROJECT_ID\n" +
