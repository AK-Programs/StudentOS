...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (code && !hasExchanged) {
      ...
      console.log("[SUPABASE OAUTH] Hash params found on mount. Setting loading states and setting up post-load hash cleanup...");
      setFirebaseLoading(true);
      setDataLoading(true);
      
      // Clear hash after a short delay so Supabase client has time to read it synchronously on instantiation
      setTimeout(() => {
        console.log("[SUPABASE OAUTH] Cleaning up URL hash fragment from browser.");
        try {
          window.history.replaceState({}, document.title, window.location.pathname);
        } catch (e) {
          console.error(e);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (isAuthInProgress && !session) {
        ...
        // Schedule a safety fallback timeout to clear loading state and stale parameters if they fail to resolve within 4 seconds
        if (!(window as any).__studentos_auth_timeout__) {
          (window as any).__studentos_auth_timeout__ = setTimeout(() => {
            console.warn("[SUPABASE AUTH] OAuth redirect exchange timed out (possibly stale token in URL). Cleaning up URL and loading state.");
            try {
              window.history.replaceState({}, document.title, window.location.pathname);
            } catch (err) {
              console.error(err);
      ...
      if (isAuthInProgress && session) {
        console.log("[SUPABASE AUTH] OAuth exchange succeeded. Stripping OAuth credentials from URL.");
        try {
          window.history.replaceState({}, document.title, window.location.pathname);
        } catch (err) {
                ...
                <div id="reg-step-landing" className="space-y-6">
                  ...
                  <form onSubmit={handleEmailSignIn} className="space-y-4">
                    ...
                    <button
                      ...
                      className="w-full py-3.5 px-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs uppercase tracking-wider transition-all shadow-lg shadow-indigo-500/25 active:scale-95 disabled:opacity-50"
                    >
