...
import { 
  INITIAL_ANNOUNCEMENTS, MOCK_QUIZZES, AI_PERSONAS, 
  INITIAL_FEEDBACK, INITIAL_MATERIALS, MOCK_SCHEDULES 
} from './mockData';

// Firebase Integrations
import { auth, db, storage, handleFirestoreError, OperationType, sendNotificationToUsers } from './firebase';
import { 
...
export default function App() {
  ...
  // Database Seeding Helper for smooth, rich out-of-the-box Experience
  const seedInitialDatabase = async () => {
    try {
      ...
      if (feedSnap.empty) {
        for (const feed of INITIAL_FEEDBACK) {
          await setDoc(doc(db, 'feedbacks', feed.id), feed);
        }
      }
  ...
  const [profileTab, setProfileTab] = useState<'overview' | 'attendance' | 'records' | 'remarks' | 'analytics'>('overview');
  ...
  const [regStep, setRegStep] = useState<'landing' | 'role' | 'profile' | 'submit'>('landing');
  ...
  const [tasks, setTasks] = useState<Task[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_tasks');
      return (cached && cached !== "undefined") ? JSON.parse(cached) : [];
    } catch (_) { return []; }
  });
  const [schedules, setSchedules] = useState<ScheduleItem[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_schedules');
      return (cached && cached !== "undefined") ? JSON.parse(cached) : [];
    } catch (_) { return []; }
  });
  const [vaultNotes, setVaultNotes] = useState<VaultNote[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_notes');
      return (cached && cached !== "undefined") ? JSON.parse(cached) : [
        {
          id: 'n-1',
          title: 'BST Balancing Rotations Study Notes',
          subject: 'Computer Science',
          content: 'AVLs require tracking balance factors (-1, 0, 1). Perform left rotation on single imbalances, left-right double rotations on zigzag patterns.',
        ...
        {
          id: 'n-2',
          title: 'Quantum Entanglement Paradigms',
          subject: 'Physics',
          content: 'Spooky action at a distance occurs when entangling spin states. Measuring state spin A instantly collapses spin state B into the opposite direction.',
          createdAt: 'Jun 9, 2026',
          icon: '⚛️',
          coverBg: 'bg-gradient-to-r from-emerald-600 to-teal-900'
        }
      ];
    } catch (_) { return []; }
  });
  ...
  const [homeworkList, setHomeworkList] = useState<Homework[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_homework');
    ...
    return [
      {
        ...
        classSection: 'Astra',
        subject: 'Physics',
        title: '🛰️ Astra Section - Satellite Orbit Vector Calculations',
        content: `### Objective:\nCalculate the orbital velocity and gravitational vector pull of a satellite in Low Earth Orbit (LEO).\n\n### Core Formulas:\n- Velocity: \`v = sqrt(G * M / r)\`\n- Orbit height: \`r = R_E + h\` \n\n### Deliverables:\n1. Solve for altitudes of $450\\text{ km}$ and $800\\text{ km}$.\n2. Highlight how atmospheric drag intersects orbital lifespan.`,
        dueDate: 'Jun 14, 2026',
        givenBy: 'Prof. Elara',
        createdAt: 'Jun 9, 2026',
        completedList: []
      },
      {
        ...
        classSection: 'Elera',
        subject: 'Computer Science',
        title: '⚡ Elera Section - Asynchronous Network Packet Shaper',
        content: `### Objective:\nDesign a simulated packet shaping routine for high-throughput node topologies.\n\n### Core Architecture:\n- Handle buffering bottlenecks using standard FIFO queues.\n- Prevent packet drops under burst traffic loads.\n\n### Target:\nExplain the sliding window algorithm and trace throughput levels in logs.`,
        dueDate: 'Jun 16, 2026',
        givenBy: 'Dr. Ruby',
        createdAt: 'Jun 8, 2026',
        completedList: []
      },
      {
        ...
        classSection: 'Solara',
        subject: 'Chemistry',
        title: '☀️ Solara Section - Photovoltaic Cell Efficiency Analysis',
        content: `### Objective:\nEvaluate band-gap threshold materials in dye-sensitized PV semiconductor platforms.\n\n### Formulas:\n- Maximum Power output: \`P_max = V_oc * I_sc * FF\`\n\n### Tasks:\nCalculate photon energy ratios and submit your performance metrics sheet.`,
        dueDate: 'Jun 18, 2026',
        givenBy: 'Principal Mitchell',
        createdAt: 'Jun 9, 2026',
        completedList: []
      },
      {
        ...
        classSection: 'Vega',
        subject: 'Mathematics',
        title: '🌌 Vega Section - Deep-Space Signal Frequency Trigonometry',
        content: `### Instructions:\nModel cosmic microwave resonance vectors using Fourier sine-wave equations.\n\n### Problem Set:\n- Solve for periodic variations in noise levels: \`f(t) = A * sin(ωt + φ)\`\n\n### Goal:\nDetermine wave resonance nodes using vectors coordinates.`,
        dueDate: 'Jun 20, 2026',
        givenBy: 'Instructor Knox',
        createdAt: 'Jun 9, 2026',
        completedList: []
      },
      {
        ...
        classSection: 'Solara',
        subject: 'Physics',
        title: '📐 Solve Rotational Mechanics Vector Problems',
        content: `### Objective:\nSolve exercises 4 through 9 on page 142 of your Physics textbook.\n\n### Core Formulas:\n- Torque: \`τ = r × F\`\n- Angular Momentum: \`L = I × ω\`\n\n### Requirements:\n1. Show vector diagrams for each calculation.\n2. Submit as a PDF outline.`,
        dueDate: 'Tomorrow',
        givenBy: 'Prof. Elara',
        createdAt: 'Jun 8, 2026',
        completedList: []
  ...
  const [announcements, setAnnouncements] = useState<any[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_announcements');
      return (cached && cached !== "undefined") ? JSON.parse(cached) : [];
    } catch (_) { return []; }
  });
  ...
  const [savedMaterialIds, setSavedMaterialIds] = useState<string[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_saved_materials');
      return (cached && cached !== "undefined") ? JSON.parse(cached) : [];
    } catch (_) { return []; }
  });
  ...
  const [materials, setMaterials] = useState<MaterialResource[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_materials');
      return (cached && cached !== "undefined") ? JSON.parse(cached) : [];
    } catch (_) { return []; }
  });
  const [feedbackPosts, setFeedbackPosts] = useState<FeedbackPost[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_feedbacks');
      return (cached && cached !== "undefined") ? JSON.parse(cached) : [];
    } catch (_) { return []; }
  });
  const [students, setStudents] = useState<UserProfile[]>(() => {
    try {
      const cached = localStorage.getItem('s_os_students');
      if (cached && cached !== "undefined") return JSON.parse(cached);
    } catch (_) {}
    return [
      {
      ...
      },
      {
        ...
        quizzesTaken: 3,
        streakDays: 5,
        specialtySubject: 'Computer Science'
      },
      {
        name: 'Meera Jain',
  ...
  useEffect(() => {
    const cachedActive = localStorage.getItem(`s_os_ai_active_id_${auth.currentUser?.uid}`);
    if (cachedActive && auth.currentUser) setActiveThreadId(cachedActive);
  }, [auth.currentUser?.uid]);

  // Sync active thread ID locally
  useEffect(() => {
    if (auth.currentUser?.uid) {
      localStorage.setItem(`s_os_ai_active_id_${auth.currentUser.uid}`, activeThreadId);
    }
      ...
      setNotifications(prev => {
        ...
        list.forEach(n => {
          if (!prevIds.has(n.id) && !n.read) {
            showNotification(`${getNotificationEmoji(n.type)} ${n.title}\n${n.message}`);
          }
        });
        return list;
      });
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        setFirebaseOnboardingUser(firebaseUser);
        try {
          ...
          const profileData = docExists ? userDoc.data() as UserProfile : null;
          const currentRole = profileData ? profileData.role : 'null';
          const currentStatus = profileData ? profileData.accountStatus : 'null';

          console.log('==================================================');
          console.log('SUPER ADMIN AUTO-LOGIN INVESTIGATION');
          console.log(`Current User UID: ${firebaseUser.uid}`);
          console.log(`Current Email: ${firebaseUser.email}`);
          console.log(`Current Firestore Role: ${currentRole}`);
          console.log(`Current Firestore Path: users/${firebaseUser.uid}`);
          console.log(`Operation Type: None (Read only)`);
          console.log(`Document exists? ${docExists ? 'YES' : 'NO'}`);
          console.log(`Current role? ${currentRole}`);
          console.log(`Current accountStatus? ${currentStatus}`);
          console.log('==================================================');
          ...
          if (docExists) {
            ...
            if (isWhitelisted && (profile.role !== 'super_admin' || profile.accountStatus !== 'approved')) {
              ...
              console.log('==================================================');
              console.log('DEBUG OUTPUT (UPDATING REGISTERED USER TO SUPER ADMIN)');
              console.log(`Current User UID: ${firebaseUser.uid}`);
              console.log(`Current Email: ${firebaseUser.email}`);
              console.log(`Current Firestore Role: ${currentRole}`);
              console.log(`Current Firestore Path: users/${firebaseUser.uid}`);
              console.log(`Operation Type: setDoc (merge: merge: true)`);
              console.log(`Document exists? YES`);
              console.log(`Current role? ${currentRole}`);
              console.log(`Current accountStatus? ${currentStatus}`);
              console.log(`Exact Failing Line: await setDoc(doc(db, 'users', firebaseUser.uid), profile, { merge: true })`);
              console.log('==================================================');
            ...
            showNotification(`Access credentials validated. Welcome back, ${profile.name}!`);
          } else {
            if (isWhitelisted) {
              ...
              console.log('==================================================');
              console.log('DEBUG OUTPUT (CREATING NEW SUPER ADMIN USER)');
              console.log(`Current User UID: ${firebaseUser.uid}`);
              console.log(`Current Email: ${firebaseUser.email}`);
              console.log(`Current Firestore Role: null`);
              console.log(`Current Firestore Path: users/${firebaseUser.uid}`);
              console.log(`Operation Type: setDoc (create)`);
              console.log(`Document exists? NO`);
              console.log(`Current role? null`);
              console.log(`Current accountStatus? null`);
              console.log(`Exact Failing Line: await setDoc(doc(db, 'users', firebaseUser.uid), newProfile)`);
              console.log('==================================================');

              await setDoc(doc(db, 'users', firebaseUser.uid), newProfile);
              setCurrentUser(newProfile);
              updateRecentAccounts(newProfile);
              showNotification(`Super Admin initialized. Welcome to StudentOS, ${newProfile.name}!`);
            } else {
              // Initiate onboarding phase
              setRegName(firebaseUser.displayName || '');
              setRegEmail(firebaseUser.email || '');
              setIsGoogleLinked(true);
              setLinkedGoogleUid(firebaseUser.uid);
              setLinkedGooglePhoto(firebaseUser.photoURL || '');
              setRegStep('role');
              setCurrentUser(null);
            }
          }
        } catch (err) {
          console.error('[Firebase Auth] Error reading user record:', err);
        }
      } else {
        ...
        if (cached && cached !== "undefined") {
          try {
            const parsed = JSON.parse(cached);
            if (parsed && parsed.email && !parsed.uid?.startsWith('google:')) {
  ...
  // Real-time Firestore Subscriptions for User Node Workspace
  useEffect(() => {
    if (!currentUser || !auth.currentUser) return;
    const uid = auth.currentUser.uid;

    // 1. Task List Sync
    console.log("FIRESTORE CALL", "tasks", "onSnapshot", uid, currentUser?.role);
    const unsubscribeTasks = onSnapshot(
      query(collection(db, 'tasks'), where('userId', '==', uid)),
      (snapshot) => {
        const list: Task[] = [];
        snapshot.forEach((doc) => {
          list.push({ ...doc.data() as Task, id: doc.id });
        });
        setTasks(list);
      },
      (error) => {
        console.error("Firestore error on tasks list snapshot:", error);
        handleFirestoreError(error, OperationType.LIST, 'tasks');
      }
    );

    // 2. Vault Notes Sync
    console.log("FIRESTORE CALL", "notes", "onSnapshot", uid, currentUser?.role);
    const unsubscribeNotes = onSnapshot(
      query(collection(db, 'notes'), where('userId', '==', uid)),
      (snapshot) => {
        const list: VaultNote[] = [];
        snapshot.forEach((doc) => {
          list.push({ ...doc.data() as VaultNote, id: doc.id });
        });
        setVaultNotes(list);
      },
      (error) => {
        console.error("Firestore error on notes list snapshot:", error);
        handleFirestoreError(error, OperationType.LIST, 'notes');
      }
    );

    // 2.5 AI Buddy Chats Sync
    console.log("FIRESTORE CALL", "aiBuddyChats", "onSnapshot", uid, currentUser?.role);
    const unsubscribeAiChats = onSnapshot(
      query(collection(db, 'aiBuddyChats'), where('userId', '==', uid)),
      (snapshot) => {
        if (!snapshot.empty) {
          const list: any[] = [];
          snapshot.forEach((doc) => {
          ...
          list.sort((a,b) => (b.createdAt || 0) - (a.createdAt || 0));
          setAiThreads(list as any);

          // Auto-migrate activeThreadId if it is 'thread-default' to the user-specific default ID
          setActiveThreadId(prev => {
            if (prev === 'thread-default') {
              const hasUserDefault = list.find(t => t.id === `thread-default-${uid}`);
              return hasUserDefault ? `thread-default-${uid}` : (list[0]?.id || prev);
            }
            return prev;
          });
        } else {
          ...
          const defaultT = {
            ...
            mode: 'explanatory',
            messages: [
              { role: 'assistant', content: "Hey! 🚀 Welcome to your Campus AI workspace. Here is what you can do:\n\n- **Start New Chat Threads**: Manage different lesson conversations seamlessly.\n- **Toggle Dynamic Learning Modes**: Socratic guidance, deeper explanations, code coach reviews, and exam challenges.\n- **Analyze Uploaded Files**: Drag and drop or browse standard codes, notes, chemistry formulae, or texts here to direct our context!\n\nWhat can I clarify today?" }
            ],
            attachedFile: null,
            userId: uid,
            createdAt: Date.now()
          };
          console.log("FIRESTORE CALL", "aiBuddyChats", "setDoc", uid, currentUser?.role);
          setDoc(doc(db, 'aiBuddyChats', userDefaultId), defaultT)
            .then(() => {
              setActiveThreadId(userDefaultId);
            })
            .catch(error => {
              console.error("Firestore error setting initial AI buddy chat:", error);
              throw error;
            });
        }
      },
      (error) => {
        console.error("Firestore error on aiBuddyChats list snapshot:", error);
        handleFirestoreError(error, OperationType.LIST, 'aiBuddyChats');
      }
    );

    // 3. Educational Materials Hub Sync
    console.log("FIRESTORE CALL", "materials", "onSnapshot", uid, currentUser?.role);
    const unsubscribeMaterials = onSnapshot(
      collection(db, 'materials'),
      (snapshot) => {
        const list: MaterialResource[] = [];
        snapshot.forEach((doc) => {
          list.push({ ...doc.data() as MaterialResource, id: doc.id });
        });
        setMaterials(list);
      },
      (error) => {
        console.error("Firestore error on materials collection snapshot:", error);
        handleFirestoreError(error, OperationType.LIST, 'materials');
      }
    );

    // 4. Feedback posts Sync
    console.log("FIRESTORE CALL", "feedbacks", "onSnapshot", uid, currentUser?.role);
    const unsubscribeFeedbacks = onSnapshot(
      collection(db, 'feedbacks'),
      (snapshot) => {
        const list: FeedbackPost[] = [];
        snapshot.forEach((doc) => {
          list.push({ ...doc.data() as FeedbackPost, id: doc.id });
        });
        setFeedbackPosts(list);
      },
      (error) => {
        console.error("Firestore error on feedbacks collection snapshot:", error);
        handleFirestoreError(error, OperationType.LIST, 'feedbacks');
      }
    );

    // 6. Announcements Sync
    console.log("FIRESTORE CALL", "announcements", "onSnapshot", uid, currentUser?.role);
    const unsubscribeAnnouncements = onSnapshot(
      ...
      (error) => {
        console.error("Firestore error on announcements collection snapshot:", error);
        handleFirestoreError(error, OperationType.LIST, 'announcements');
      }
    );

    // 7. Chats Sync
    console.log("FIRESTORE CALL", "chats", "onSnapshot", uid, currentUser?.role);
    const unsubscribeChats = onSnapshot(
      ...
      (error) => {
        console.error("Firestore error on chats collection snapshot:", error);
    ...
    const establishConnection = () => {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          switch (payload.type) {
            case 'sync:state':
              if (payload.state.chats) setChats(payload.state.chats);
  ...
  useEffect(() => {
    localStorage.setItem('s_os_announcements', JSON.stringify(announcements));
  }, [announcements]);
    ...
    try {
      ...
      if (resultText) {
        if (selectedTextLength > 0) {
          ...
          setVaultNotes(prev => prev.map(n => n.id === activeNote.id ? { ...n, content: updatedContent } : n));
        } else {
  ...
  // Save changes to localStorage on modifying critical parameters
  useEffect(() => {
    localStorage.setItem('s_os_tasks', JSON.stringify(tasks));
  }, [tasks]);

  useEffect(() => {
    localStorage.setItem('s_os_notes', JSON.stringify(vaultNotes));
  }, [vaultNotes]);

  useEffect(() => {
    localStorage.setItem('s_os_schedules', JSON.stringify(schedules));
  }, [schedules]);

  useEffect(() => {
    localStorage.setItem('s_os_materials', JSON.stringify(materials));
  }, [materials]);

  useEffect(() => {
    localStorage.setItem('s_os_feedbacks', JSON.stringify(feedbackPosts));
  }, [feedbackPosts]);

  useEffect(() => {
    localStorage.setItem('s_os_students', JSON.stringify(students));
  }, [students]);

  useEffect(() => {
    localStorage.setItem('s_os_homework', JSON.stringify(homeworkList));
  }, [homeworkList]);

  useEffect(() => {
    localStorage.setItem('s_os_saved_materials', JSON.stringify(savedMaterialIds));
  }, [savedMaterialIds]);
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (firebaseUser) {
        ...
        showNotification(`Signed in as ${firebaseUser.displayName}! Constructing StudentOS environment.`);
      }
    } catch (error: any) {
      console.error('[Firebase Auth Debug] Google Sign-In failed:', error);
  ...
  const handleGoogleSignInWithSelect = async () => {
    try {
      ...
      console.error('[Firebase Auth Debug] Google Sign-In with select_account failed:', error);
      const code = error?.code || 'unknown';
      const msg = error?.message || String(error);
      setAuthError({
        code,
  ...
  const handleLinkGoogle = async () => {
    try {
      ...
      if (firebaseUser) {
        ...
        setLinkedGoogleUid(firebaseUser.uid);
        setLinkedGooglePhoto(firebaseUser.photoURL || '');
        showNotification(`✓ Google Account linked: ${firebaseUser.displayName}! Fields pre-filled.`);
      }
    } catch (error: any) {
      console.error('[Firebase Auth Debug] Link Google failed:', error);
      const code = error?.code || 'unknown';
      const msg = error?.message || String(error);
      setAuthError({
        code,
  ...
  const handleContinueRecentAccount = async (acc: RecentAccount) => {
    if (acc.uid && acc.uid.startsWith('manual-')) {
      if (acc.role === 'admin' || acc.role === 'coordinator') {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            showNotification('Profile not found in database.');
          }
        } catch (e) {
          console.error(e);
  ...
  const performGoogleAuthForRecentAccount = async (acc: RecentAccount) => {
    try {
      ...
      const firebaseUser = result.user;
      if (firebaseUser) {
        if (firebaseUser.email !== acc.email) {
          showNotification(`Mismatch: Signed in as ${firebaseUser.email}, but expected ${acc.email}.`);
        } else {
          showNotification(`Authenticated successfully as ${acc.name}!`);
        }
      }
    } catch (error: any) {
      console.error('[Firebase Auth Debug] Recent account Google authentication failed:', error);
      const code = error?.code || 'unknown';
      const msg = error?.message || String(error);
      setAuthError({
        code,
  ...
  const handleVerifyPinAndLogin = async () => {
    ...
    try {
      const userDoc = await getDoc(doc(db, 'users', securityVerificationAcc.uid));
      if (userDoc.exists()) {
        const profile = userDoc.data() as UserProfile;
  ...
  const handleOnboardingLogin = async (e: React.FormEvent) => {
    ...
    const uid = linkedGoogleUid || `manual-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
    ...
    if (regRole === 'student') {
      ...
      newProfile.grade = 'Grade 10';
      newProfile.section = 'Solara';
      newProfile.house = 'Ruby';
  ...
  const handleAddTask = async () => {
    ...
    try {
      await setDoc(doc(db, 'tasks', taskId), item);
      setNewTaskTitle('');
      showNotification('New task milestone appended to cloud repository.');
    } catch (err) {
  ...
  const toggleTask = async (id: string) => {
    ...
    if (!found) return;
    try {
      const nextState = !found.completed;

      await updateDoc(doc(db, 'tasks', id), { completed: nextState });
    } catch (err) {
  ...
  const deleteTask = async (id: string) => {
    try {
      ...
      showNotification('Task deleted from cloud node.');
    } catch (err) {
  ...
  const handleSaveNote = async () => {
    if (!newNoteTitle.trim() || !newNoteContent.trim() || !auth.currentUser) {
    ...
    try {
      ...
      setNewNoteTitle('');
      setNewNoteContent('');
      showNotification('Lecture notes secured in Vault Cloud.');
    } catch (err) {
  ...
  const handleDeleteNote = async (id: string) => {
    try {
      ...
      showNotification('Lecture note removed from repository.');
    } catch (err) {
  ...
  const handleCreateNewNote = async () => {
    ...
    try {
      ...
      showNotification('Notion-like study canvas instantiated!');
    } catch (err) {
  ...
  const handleDownloadMaterial = async (mat: MaterialResource) => {
    showNotification('📥 Download Manager is Coming Soon / Under Development.');
    return;
  };
  ...
  const handleSendChat = async (overrideText?: string, materialIdId?: string) => {
    ...
    try {
      await setDoc(doc(db, 'chats', msgId), msg);
      if (!overrideText) {
        setNewChatText('');
      }
      setTimeout(() => {
      ...
      handleFirestoreError(err, OperationType.CREATE, `chats/${msgId}`);
    }
  };
  ...
  const handleCreateRoom = (e: React.FormEvent) => {
    ...
    try {
      localStorage.setItem('s_os_chat_rooms', JSON.stringify(updated));
    } catch (_) {}
  ...
  const handleCreateNewThread = () => {
    ...
    if (auth.currentUser?.uid) {
      console.log("FIRESTORE CALL", "aiBuddyChats", "setDoc", auth.currentUser.uid, currentUser?.role);
      setDoc(doc(db, 'aiBuddyChats', newId), newThread)
        .catch(error => {
          alert(
            "Firestore Error\n" +
            "Collection: aiBuddyChats\n" +
            "Document: " + newId + "\n" +
            "Operation: setDoc\n" +
  ...
  const handleSwitchPersona = (personaId: string) => {
    ...
    if (auth.currentUser?.uid) {
       ...
       if (t) {
         console.log("FIRESTORE CALL", "aiBuddyChats", "setDoc", auth.currentUser.uid, currentUser?.role);
         setDoc(doc(db, 'aiBuddyChats', t.id), { ...t, personaId, title: `Study with ${AI_PERSONAS.find(p => p.id === personaId)?.name || 'Tutor'}` }, { merge: true })
           .catch(error => {
              alert(
                "Firestore Error\n" +
                "Collection: aiBuddyChats\n" +
  ...
  const handleSwitchMode = (mode: 'explanatory' | 'socratic' | 'coder' | 'quiz_gen') => {
    ...
    if (auth.currentUser?.uid) {
       ...
       if (t) {
         console.log("FIRESTORE CALL", "aiBuddyChats", "setDoc", auth.currentUser.uid, currentUser?.role);
         setDoc(doc(db, 'aiBuddyChats', t.id), { ...t, mode }, { merge: true })
           .catch(error => {
              alert(
                "Firestore Error\n" +
                "Collection: aiBuddyChats\n" +
  ...
  const handleDeleteThread = (threadId: string, e: React.MouseEvent) => {
    ...
    if (auth.currentUser?.uid) {
      console.log("FIRESTORE CALL", "aiBuddyChats", "deleteDoc", auth.currentUser.uid, currentUser?.role);
      deleteDoc(doc(db, 'aiBuddyChats', threadId))
        .catch(error => {
          alert(
            "Firestore Error\n" +
            "Collection: aiBuddyChats\n" +
  ...
  const handleAskAIModel = async () => {
    ...
    if (auth.currentUser?.uid) {
       console.log("FIRESTORE CALL", "aiBuddyChats", "setDoc", auth.currentUser.uid, currentUser?.role);
       setDoc(doc(db, 'aiBuddyChats', targetThreadId), { 
         ...currentThread, 
         id: targetThreadId,
         userId: auth.currentUser.uid,
         title: newTitle,
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 font-sans ${lightMode ? 'bg-slate-50 text-slate-800' : 'bg-slate-950 text-slate-100'}`}>
      ...
      {/* Startup Screen */}
      {showStartup && (
        <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center z-[100] transition-opacity duration-500 animate-fadeIn">
          <div className="max-w-md w-full px-8 text-center space-y-6">
            {/* Logo */}
            <div className="relative inline-flex flex-col items-center gap-2">
              <div className="h-20 w-20 rounded-3xl bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center text-4xl shadow-2xl animate-pulse">
                🎓
              </div>
              <h1 className="text-4xl font-black tracking-tight text-white font-display">StudentOS</h1>
              <span className="text-[10px] tracking-widest uppercase font-mono text-indigo-400">Initializing terminal</span>
            </div>
            ...
            <div className="space-y-3 pt-4">
              <div className="flex items-center justify-center gap-2.5">
                ...
                <p className="text-xs text-slate-300 font-medium font-mono min-h-[16px] transition-all duration-300">
                  {STARTUP_MESSAGES[loadingStep]}
                </p>
              </div>

              {/* Progress bar container */}
              <div className="w-full bg-slate-900 border border-white/5 rounded-full h-2 overflow-hidden shadow-inner p-[1px]">
                <div 
              ...
              <div className="flex justify-between items-center text-[10px] font-mono text-slate-500">
                <span>BUILD TEST 30-JUNE-2026</span>
      ...
      {!currentUser && (
        <div id="onboarding-gate-modal" className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 overflow-y-auto flex items-start sm:items-center justify-center p-4">
          <div className="smart-glass max-w-xl w-full p-6 sm:p-8 my-auto rounded-3xl border border-white/10 space-y-6 animate-fadeIn">
            ...
            <div className="text-center space-y-2">
              ...
              <h1 className="text-3xl font-black font-display tracking-tight text-white">StudentOS</h1>
              <p className="text-sm text-slate-400">Collaborative academic dashboard workspace and identity node.</p>
              <div className="inline-block mt-1 px-2.5 py-1 text-[9px] font-mono font-bold tracking-widest text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 rounded-full">
                BUILD TEST 30-JUNE-2026
              </div>
            </div>

            <div className="space-y-6">
              {regStep === 'landing' && (
                <div id="reg-step-landing" className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2.5 py-3.5 px-4 rounded-2xl bg-white text-slate-900 font-extrabold text-xs uppercase tracking-wider hover:bg-slate-100 transition-all shadow-md active:scale-95"
                    >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="py-2.5 px-3 text-[10px] font-black uppercase tracking-wider bg-slate-900 hover:bg-slate-800 text-slate-300 rounded-xl transition-all border border-white/5"
                      >
                        🔄 Switch Google
                      </button>
                      <button
                  ...
                  {authError && (
                    <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-left space-y-3 animate-fadeIn">
                      <div className="flex gap-2.5 items-start">
                        ...
                        <div>
                          <h4 className="text-xs font-extrabold text-rose-400 uppercase tracking-wide">Firebase Authentication Diagnostic</h4>
                      ...
                      <div className="pt-2.5 border-t border-rose-500/10 space-y-2">
                        <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Recommended Actions to Resolve:</h5>
                        
                        {authError.code === 'auth/unauthorized-domain' ? (
                          <div className="text-[11px] text-slate-300 space-y-1 bg-slate-950/40 p-2.5 rounded-xl border border-white/5">
                            <p className="font-semibold text-amber-400">🌐 Domain Whitelist Required:</p>
                            <p className="text-slate-400">The domain <code className="text-white px-1 py-0.5 bg-slate-800 rounded">{authError.hostname}</code> is not whitelisted in your Firebase project.</p>
                            <ol className="list-decimal pl-4 space-y-1 mt-1.5 text-slate-400">
                              <li>Go to <a href="https://console.firebase.google.com/" target="_blank" rel="noopener noreferrer" className="text-indigo-400 underline hover:text-indigo-300">Firebase Console</a></li>
                              <li>Go to <span className="font-bold">Authentication</span> &rarr; <span className="font-bold">Settings</span> &rarr; <span className="font-bold">Authorized domains</span></li>
                              <li>Click <span className="font-bold">Add domain</span> and enter: <code className="text-indigo-300 font-mono select-all bg-indigo-500/10 px-1 py-0.5 rounded">{authError.hostname}</code></li>
                            </ol>
                          </div>
                        ) : authError.code === 'auth/operation-not-allowed' ? (
                          <div className="text-[11px] text-slate-300 space-y-1 bg-slate-950/40 p-2.5 rounded-xl border border-white/5">
                            <p className="font-semibold text-amber-400">🔑 Google Provider Disabled:</p>
                            <p className="text-slate-400">Google Sign-In has not been enabled in your Firebase Project.</p>
                            <ol className="list-decimal pl-4 space-y-1 mt-1.5 text-slate-400">
                              <li>Go to <a href="https://console.firebase.google.com/" target="_blank" rel="noopener noreferrer" className="text-indigo-400 underline hover:text-indigo-300">Firebase Console</a></li>
                              <li>Go to <span className="font-bold">Authentication</span> &rarr; <span className="font-bold">Sign-in method</span></li>
                              <li>Click <span className="font-bold">Add new provider</span> &rarr; <span className="font-bold">Google</span></li>
                              <li>Toggle <span className="font-bold">Enable</span>, select your support email, and click <span className="font-bold">Save</span>.</li>
                            </ol>
                          </div>
                        ) : (
                          <div className="text-[11px] text-slate-300 space-y-1 bg-slate-950/40 p-2.5 rounded-xl border border-white/5">
                            <p className="font-semibold text-amber-400">💡 General Check:</p>
                            <ul className="list-disc pl-4 space-y-1 mt-1 text-slate-400">
                              <li>Verify that your Firebase database, auth, and API keys are active.</li>
                              <li>If popups are disabled or cookies are blocked, allow them for this domain and reload.</li>
                              <li>Add <code className="text-white bg-slate-800 px-1 py-0.5 rounded">{authError.hostname}</code> to your Firebase authorized domains list.</li>
                            </ul>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="pt-4 border-t border-white/5 text-center space-y-3">
                    ...
                    <button
                      ...
                      className="w-full py-3.5 px-4 rounded-2xl bg-indigo-600/20 border border-indigo-500/30 text-indigo-400 font-extrabold text-xs uppercase tracking-wider hover:bg-indigo-600 hover:text-white transition-all active:scale-95 shadow-md animate-pulse"
                    >
              ...
              {regStep === 'role' && (
                <div id="reg-step-role" className="space-y-5 animate-fadeIn">
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase font-black tracking-widest text-indigo-400">Step 1 of 3</span>
                    <h2 className="text-xl font-bold text-white">Choose Account Type</h2>
                    <p className="text-xs text-slate-400">Select the primary profile identity you want to request.</p>
                  </div>

                  <div className="grid grid-cols-1 gap-2.5">
                    {[
                      { type: 'student', label: '🎓 Student', desc: 'Immediate workspace exploration access' },
                      ...
                      <button
                        ...
                        className={`p-4 rounded-2xl text-left border transition-all flex flex-col gap-1 ${
                          regRole === item.type
                            ? 'bg-indigo-600/20 border-indigo-500 text-white shadow-lg shadow-indigo-500/10'
                            : 'bg-slate-900/60 border-white/5 text-slate-300 hover:border-white/10 hover:bg-slate-900'
                        }`}
                      >
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-white/5">
                    ...
                    <button
                      ...
                      className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-extrabold uppercase tracking-wider transition-all"
                    >
                      Next →
                    </button>
                  </div>
              ...
              {regStep === 'profile' && (
                <form id="reg-step-profile" onSubmit={(e) => { e.preventDefault(); setRegStep('submit'); }} className="space-y-5 animate-fadeIn">
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase font-black tracking-widest text-indigo-400">Step 2 of 3</span>
                  ...
                  <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/20 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="p-2.5 rounded-xl bg-slate-900/60 border border-white/5 text-left space-y-1">
                        <span className="text-[9px] uppercase font-black text-slate-400 tracking-wide block">Option A</span>
                        <p className="text-xs font-bold text-white">Manual Typing</p>
                        <p className="text-[10px] text-slate-500">Enter custom name and email tag directly.</p>
                      </div>
                      <button
                        type="button"
                        onClick={handleLinkGoogle}
                        className={`p-2.5 rounded-xl text-left space-y-1 border transition-all ${
                          isGoogleLinked
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full text-sm px-4 py-3 rounded-xl bg-slate-900 border border-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-white font-medium"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-400">School Email</label>
                        <input
                          type="email"
                          required
                          value={regEmail}
                          ...
                          className="w-full text-sm px-4 py-3 rounded-xl bg-slate-900 border border-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-white font-mono"
                        />
                      </div>
                    </div>
                    ...
                    {regRole === 'student' && (
                      <div className="space-y-3 p-3.5 rounded-2xl bg-slate-900/40 border border-white/5 space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          ...
                          <div className="space-y-1">
                            <label className="text-xs font-medium text-slate-400">House</label>
                            <select
                              value={regHouse}
                        ...
                        <div className="space-y-1">
                          <label className="text-xs font-medium text-slate-400 font-sans">Profile Photo URL (Optional)</label>
                          <input
                            type="url"
                            value={regPhoto}
                            onChange={(e) => setRegPhoto(e.target.value)}
                            placeholder="https://images.unsplash.com/..."
                            className="w-full text-sm px-4 py-2.5 rounded-xl bg-slate-950 border border-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-white"
                          />
                        </div>
                      </div>
                    )}
                    ...
                    {regRole === 'coordinator' && (
                      <div className="space-y-3 p-3.5 rounded-2xl bg-slate-900/40 border border-white/5">
                    ...
                    {regRole === 'admin' && (
                      <div className="space-y-3 p-3.5 rounded-2xl bg-slate-900/40 border border-white/5">
                        ...
                        <div className="space-y-1">
                          <label className="text-xs font-medium text-slate-400">Designation</label>
                          <input
                            type="text"
                            required
              ...
              {regStep === 'submit' && (
                <div id="reg-step-submit" className="space-y-5 animate-fadeIn">
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase font-black tracking-widest text-indigo-400">Step 3 of 3</span>
                    <h2 className="text-xl font-bold text-white">Review Profile Registration</h2>
                    <p className="text-xs text-slate-400">Please verify details before creating your node.</p>
                  </div>

                  <div className="p-4 rounded-2xl bg-slate-900/80 border border-white/5 space-y-3.5 text-xs">
                    <div className="flex justify-between items-center py-1 border-b border-white/5">
                      <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">Target Account Role</span>
                      <span className="font-extrabold text-indigo-400 uppercase tracking-wide">{regRole}</span>
                    </div>
                    <div className="flex justify-between items-center py-1 border-b border-white/5">
                      <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">Full Name</span>
                      <span className="font-bold text-white">{regName}</span>
                    </div>
                    <div className="flex justify-between items-center py-1 border-b border-white/5">
                      <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">School Email</span>
                    ...
                    {regRole === 'student' && (
                      <>
                        <div className="flex justify-between items-center py-1 border-b border-white/5">
                          <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">Grade & Section</span>
                          <span className="font-bold text-white">{regGrade} - {regSection}</span>
                        </div>
                        <div className="flex justify-between items-center py-1 border-b border-white/5">
                          <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">House Alliance</span>
                    ...
                    {regRole === 'teacher' && (
                      <>
                        <div className="flex justify-between items-center py-1 border-b border-white/5">
                          <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">Department</span>
                          <span className="font-bold text-white">{regDept}</span>
                        </div>
                        <div className="flex justify-between items-center py-1 border-b border-white/5">
                          <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">Assigned Subjects</span>
                    ...
                    {regRole === 'admin' && (
                      <div className="flex justify-between items-center py-1 border-b border-white/5">
                        <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">Designation</span>
                        <span className="font-bold text-white">{regDesignation}</span>
                      </div>
                    )}

                    <div className="flex justify-between items-center py-1">
                      <span className="text-slate-400 uppercase font-black tracking-widest text-[10px]">Google Verification Linked</span>
                      <span className={`font-black tracking-wider uppercase ${isGoogleLinked ? 'text-emerald-400' : 'text-amber-500'}`}>
                        {isGoogleLinked ? 'Linked (Secure)' : 'Unlinked (Manual)'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-white/5">
                    ...
                    <button
                      ...
                      className="px-6 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-extrabold uppercase tracking-wider transition-all shadow-lg shadow-indigo-500/25 hover:scale-[1.01]"
                    >
        ...
        <div className="flex min-h-screen relative">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {sidebarOpen && (
                    <div className="flex flex-col">
                      <span className="font-extrabold text-white text-base tracking-tight leading-none">StudentOS</span>
                      <span className="text-[10px] text-indigo-400 uppercase font-black tracking-widest mt-0.5">V2.0 LTS</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <nav className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'dashboard' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>🏠</span>
                  ...
                  {effectiveRole !== 'student' && (
                    <button 
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'panel_mode' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                    >
                      <span>🖥️</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'tasks' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>📋</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'planner' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>📅</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'notes' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>📝</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'pomodoro' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>🍅</span>
                ...
                <div className="space-y-1">
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'quiz' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>🧠</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'whiteboard' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>✏️</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'materials' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>📚</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'assignments' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>📓</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'houses' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>🏆</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'homework' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'feedback' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>💬</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'ai_teacher' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>🤖</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'peer_chat' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>🌍</span>
                ...
                <div className="space-y-1">
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'analytics' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>📊</span>
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'profile' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>👤</span>
                    {sidebarOpen && `${effectiveRole.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} Profile`}
                  </button>

                  <button 
                    onClick={() => handleTabSelect('pricingPage')}
                    className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${activeTab === 'pricingPage' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                  >
                    <span>💎</span>
                    {sidebarOpen && 'System Premium'}
                  </button>

                  {effectiveRole === 'teacher' && (
                    <button 
                      onClick={() => handleTabSelect('faculty')}
                      className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold border border-emerald-500/10 transition-all ${activeTab === 'faculty' ? 'bg-emerald-600 text-white shadow-lg' : 'text-emerald-400 hover:text-white hover:bg-emerald-500/10'}`}
                    >
                      <span>👨‍🏫</span>
                  ...
                  {['teacher', 'coordinator', 'admin', 'super_admin'].includes(effectiveRole) && (
                    <button 
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold border border-amber-500/10 transition-all ${activeTab === 'attendance_manager' ? 'bg-amber-600 text-white shadow-lg' : 'text-amber-400 hover:text-white hover:bg-amber-500/10'}`}
                    >
                      <span>📋</span>
                  ...
                  {(effectiveRole === 'admin' || effectiveRole === 'super_admin') && (
                    <button 
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-semibold border border-rose-500/10 transition-all ${activeTab === 'admin' ? 'bg-rose-600 text-white shadow-lg' : 'text-rose-400 hover:text-white hover:bg-rose-500/10'}`}
                    >
                      <span>⚙️</span>
                ...
                <div className="p-3.5 rounded-2xl bg-white/5 flex items-center gap-3">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-[10px] text-slate-400 truncate mt-1">
                      {getUserProfileTitle(currentUser, effectiveRole)}
                    </span>
          ...
          <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${sidebarOpen ? 'md:ml-64' : 'md:ml-20'}`}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-extrabold tracking-tight text-white font-display">
                    ...
                    {activeTab === 'pricingPage' && 'Premium Upgrades'}
                  </h2>
                </div>
              </div>
            ...
            <main className={`flex-1 p-6 md:p-8 max-w-7xl w-full mx-auto space-y-8 ${fontSize === 'large' ? 'text-lg' : fontSize === 'huge' ? 'text-xl' : 'text-sm'}`}>
                          ...
                          {INITIAL_ANNOUNCEMENTS.map(ann => (
                            <div key={ann.id} className="p-5 rounded-2xl bg-slate-900/60 border border-white/5 hover:border-white/10 transition-all space-y-2">
                              <div className="flex justify-between items-center flex-wrap gap-2">
                                <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${ann.tagColor}`}>{ann.tag}</span>
                                ...
                                <button
                                  onClick={() => {
                                    if (confirm('Delete this Notion study canvas page permanently?')) {
                                      setVaultNotes(prev => prev.filter(n => n.id !== activeNote.id));
              ...
              {activeTab === 'profile' && (
                <div className="smart-glass p-8 rounded-3xl space-y-6 max-w-4xl mx-auto animate-fadeIn col-span-2">
                  ...
                  {profileTab === 'enter_marks' && (
                    <StudentMarksCenter students={students} showNotification={showNotification} />
                  )}
                  ...
                  {['student_list'].includes(profileTab) && (effectiveRole === 'teacher' || effectiveRole === 'coordinator') && currentUser && (
                    <div className="space-y-6">
                  ...
                  {['student_reports', 'class_reports', 'section_reports'].includes(profileTab) && (effectiveRole === 'teacher' || effectiveRole === 'coordinator') && currentUser && (
                    <div className="space-y-6">
                      <div className="p-6 rounded-3xl bg-slate-900 border border-white/5 animate-fadeIn">
              ...
              {/* Tab: System Premium Tiers View */}
              {activeTab === 'pricingPage' && (
                <div className="space-y-6 animate-fadeIn">
                  <div className="text-center max-w-xl mx-auto space-y-2">
                    <span className="text-[11px] font-black uppercase text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 rounded-full">System Premium Tier</span>
                    <h3 className="text-3xl font-black font-display text-white tracking-tight">Active Node Upgrades</h3>
                    <p className="text-sm text-slate-400">Unlock contextual multi-modal intelligence, vectorized custom uploads, and high priority study pipelines instantly.</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pt-4">
                    {/* Tier 1 */}
                    <div className="smart-glass p-6 rounded-2xl flex flex-col justify-between shadow-sm relative border border-white/5 hover:border-white/20 transition-all">
                      <div className="space-y-4">
                        <div className="text-xs font-bold uppercase tracking-widest text-slate-400">Standard Tier</div>
                        <h4 className="text-xl font-bold text-white">Free Plan</h4>
                        <div className="text-3xl font-black text-white">₹0 <span className="text-xs text-slate-400 font-normal">/ month</span></div>
                        <ul className="space-y-2.5 text-xs text-slate-400 border-t border-white/5 pt-4">
                          <li className="flex items-center gap-2">🟢 20 AI Messages / Day</li>
                          <li className="flex items-center gap-2">🟢 5 PDF Vectors / Day</li>
                          <li className="flex items-center gap-2">🟢 5 Image Scans / Day</li>
                          <li className="flex items-center gap-2 text-slate-600 line-through">❌ Enterprise Cloud Sync</li>
                        </ul>
                      </div>
                      <button 
                        type="button"
                        onClick={() => {
                          if (currentUser) {
                            setCurrentUser({ ...currentUser, specialtySubject: 'Free tier user' });
                          }
                          showNotification('Current selected node tier: Standard Free');
                        }}
                        className="w-full mt-6 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 px-4 rounded-xl text-xs tracking-wide uppercase transition-all"
                      >
                        Plan Engaged
                      </button>
                    </div>
                    ...
                    {/* Tier 2 */}
                    <div className="smart-glass p-6 rounded-2xl flex flex-col justify-between shadow-sm relative border border-white/5 hover:border-indigo-500/30 transition-all">
                      <div className="space-y-4">
                        <div className="text-xs font-bold uppercase tracking-widest text-indigo-400">Academic Tier</div>
                        <h4 className="text-xl font-bold text-white">Student Core</h4>
                        <div className="text-3xl font-black text-white">₹99 <span className="text-xs text-slate-400 font-normal">/ month</span></div>
                        <ul className="space-y-2.5 text-xs text-slate-400 border-t border-white/5 pt-4">
                          <li className="flex items-center gap-2">🟢 Complete Chat Access</li>
                          <li className="flex items-center gap-2">🟢 Vectorized Uploads</li>
                          <li className="flex items-center gap-2">🟢 Priority Learning Streams</li>
                          <li className="flex items-center gap-2">🟢 HW Context Solver</li>
                        </ul>
                      </div>
                      <button 
                        type="button"
                        onClick={() => {
                          if (currentUser) {
                            const updated = { ...currentUser, specialtySubject: 'Core Pro Student' };
                            setCurrentUser(updated);
                            localStorage.setItem('s_os_user', JSON.stringify(updated));
                          }
                          showNotification('Core Academic tier upgraded! Enjoy higher limits!');
                        }}
                        className="w-full mt-6 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 px-4 rounded-xl text-xs tracking-wide uppercase transition-all shadow-md shadow-indigo-500/10"
                      >
                        Upgrade Node (₹99)
                      </button>
                    </div>
                    ...
                    {/* Tier 3: Most popular */}
                    <div className="smart-glass p-6 rounded-2xl flex flex-col justify-between shadow-xl relative border-2 border-indigo-500 bg-gradient-to-b from-slate-900/40 to-indigo-950/20 transform md:scale-[1.03] z-10 transition-all">
                      <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-bold text-[9px] tracking-widest px-3 py-1 rounded-full shadow-md uppercase">Most Popular</div>
                      <div className="space-y-4">
                        <div className="text-xs font-bold uppercase tracking-widest text-purple-400">Accelerated Tier</div>
                        <h4 className="text-xl font-bold text-white">Pro Node</h4>
                        <div className="text-3xl font-black text-white">₹149 <span className="text-xs text-slate-400 font-normal">/ month</span></div>
                        <ul className="space-y-2.5 text-xs text-slate-300 border-t border-white/10 pt-4">
                          <li className="flex items-center gap-2">⚡ Everything in Academic</li>
                          <li className="flex items-center gap-2">⚡ Generative Exam Center</li>
                          <li className="flex items-center gap-2">⚡ Multi-Modal Flashcards</li>
                          <li className="flex items-center gap-2">⚡ High-Speed Priority Threads</li>
                        </ul>
                      </div>
                      <button 
                        type="button"
                        onClick={() => {
                          if (currentUser) {
                            const updated = { ...currentUser, specialtySubject: 'Accelerated Pro StudentOS' };
                            setCurrentUser(updated);
                            localStorage.setItem('s_os_user', JSON.stringify(updated));
                          }
                          showNotification('Accelerated PRO Node is activated! Unlocked synthetic exams!');
                        }}
                        className="w-full mt-6 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-3 px-4 rounded-xl text-xs tracking-wide uppercase transition-all shadow-lg shadow-indigo-500/20"
                      >
                        Engage Pro (₹149)
                      </button>
                    </div>
                    ...
                    {/* Tier 4 */}
                    <div className="smart-glass p-6 rounded-2xl flex flex-col justify-between shadow-sm relative border border-white/5 hover:border-amber-500/30 transition-all">
                      <div className="space-y-4">
                        <div className="text-xs font-bold uppercase tracking-widest text-amber-500">Ultimate Tier</div>
                        <h4 className="text-xl font-bold text-white">Enterprise King</h4>
                        <div className="text-3xl font-black text-white">₹169 <span className="text-xs text-slate-400 font-normal">/ month</span></div>
                        <ul className="space-y-2.5 text-xs text-slate-400 border-t border-white/5 pt-4">
                          <li className="flex items-center gap-2">👑 Unlimited Data Exports</li>
                          <li className="flex items-center gap-2">👑 Multi-modal Token Pools</li>
                          <li className="flex items-center gap-2">👑 Tailored Workspace UI Styles</li>
                      ...
                      <button 
                        ...
                        onClick={() => {
                          if (currentUser) {
                            const updated = { ...currentUser, specialtySubject: 'Ultimate King StudentOS' };
                            setCurrentUser(updated);
                            localStorage.setItem('s_os_user', JSON.stringify(updated));
                          }
                          showNotification('Ultimate Enterprise King active. Everything has been maximized!');
                        }}
                        className="w-full mt-6 bg-slate-800 hover:bg-slate-700 text-amber-400 font-bold py-3 px-4 rounded-xl text-xs tracking-wide uppercase transition-all border border-amber-500/10 hover:border-amber-500/30"
                      >
                        Engage King (₹169)
                      </button>
                    </div>
              ...
              {activeTab === 'whiteboard' && (
                <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col animate-fadeIn select-none touch-none">
                  
                  {/* Floating Top Controls HUD */}
                  <div className="absolute top-6 left-6 right-6 z-10 flex flex-wrap items-center justify-between gap-4 bg-slate-900/90 backdrop-blur-md p-4 rounded-3xl border border-indigo-500/20 shadow-2xl">
                    <div className="flex items-center gap-4">
                      <button 
                        onClick={() => handleTabSelect('panel_mode')}
                        className="bg-slate-950 hover:bg-slate-800 text-white font-bold px-4 py-2 rounded-xl transition-all border border-white/10 flex items-center gap-2"
                      >
                      ...
                      <div className="flex items-center gap-2 bg-slate-950/50 p-1.5 rounded-xl border border-white/5">
                        <button 
                          onClick={() => { if(typeof (window as any).whiteboardPrevSlide === 'function') (window as any).whiteboardPrevSlide(); }}
                          disabled={currentSlideIndex === 0}
                          className="p-1.5 px-2 text-white hover:bg-indigo-500/20 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed text-xs font-bold transition-colors"
                        >
                          ⬅ Prev
                        </button>
                        <span className="text-white text-[10px] font-mono font-bold px-2 bg-slate-900 rounded-md py-1 border border-white/5 self-center inline-flex">
                          Slide {currentSlideIndex + 1} / {whiteboardSlides.length}
                        </span>
                        <button 
                          onClick={() => { if(typeof (window as any).whiteboardNextSlide === 'function') (window as any).whiteboardNextSlide(); }}
                        ...
                        <button
                          onClick={() => { if(typeof (window as any).whiteboardCreateSlide === 'function') (window as any).whiteboardCreateSlide(); }}
                          className="p-1.5 px-3 text-indigo-400 hover:text-white hover:bg-indigo-500/20 rounded-lg border border-indigo-500/20 ml-1 text-xs font-bold transition-colors uppercase tracking-wider title-text" title="New Slide"
                        >
                          ➕
                        </button>
                        <button
                          onClick={() => { if(typeof (window as any).whiteboardDuplicateSlide === 'function') (window as any).whiteboardDuplicateSlide(); }}
                          className="p-1.5 px-3 text-emerald-400 hover:text-white hover:bg-emerald-500/20 rounded-lg border border-emerald-500/20 text-xs font-bold transition-colors uppercase tracking-wider title-text" title="Duplicate Slide"
                        >
                          📄
                        </button>
                        <button
                          onClick={() => { if(typeof (window as any).whiteboardDeleteSlide === 'function') (window as any).whiteboardDeleteSlide(); }}
                          className="p-1.5 px-3 text-red-500 hover:text-white hover:bg-red-500/20 rounded-lg border border-red-500/20 text-xs font-bold transition-colors uppercase tracking-wider title-text" title="Delete Slide"
                        >
                          🗑️
                        </button>
                      </div>
                      <div className="hidden md:block">
                        <h3 className="text-sm font-extrabold font-display text-white tracking-widest uppercase">Smart Board Workspace</h3>
                        <p className="text-[10px] text-slate-400 font-mono">Brio-Touch Projection Engine Active</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 bg-slate-950/50 p-1.5 rounded-2xl border border-white/5">
                      {/* Extended Tools Menu */}
                      <div className="flex bg-slate-950 px-2 py-1.5 rounded-2xl border border-white/5 gap-0.5 items-center">
                        <button 
                          onClick={() => { setCanvasTool('pen'); setCanvasShape('free'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'pen' && canvasShape === 'free' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Freehand Pencil"
                        >
                          <PenTool className="w-4 h-4" />
                        </button>
                        <button 
                          onClick={() => { setCanvasTool('marker'); setCanvasShape('free'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'marker' && canvasShape === 'free' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Marker"
                        >
                          🖍️
                        </button>
                        <button 
                          onClick={() => { setCanvasTool('brush'); setCanvasShape('free'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'brush' && canvasShape === 'free' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Brush"
                        >
                          🖌️
                        </button>
                        <button 
                          onClick={() => { setCanvasTool('fountain'); setCanvasShape('free'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'fountain' && canvasShape === 'free' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Fountain Pen"
                        >
                          🖋️
                        </button>
                        ...
                        <button 
                          onClick={() => { setCanvasTool('pen'); setCanvasShape('rect'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'pen' && canvasShape === 'rect' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Rectangle"
                        >
                          🔳
                        </button>
                        <button 
                          onClick={() => { setCanvasTool('pen'); setCanvasShape('circle'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'pen' && canvasShape === 'circle' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Circle"
                        >
                          ⭕
                        </button>
                        <button 
                          onClick={() => { setCanvasTool('pen'); setCanvasShape('triangle'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'pen' && canvasShape === 'triangle' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Triangle"
                        >
                          🔺
                        </button>
                        <button 
                          onClick={() => { setCanvasTool('pen'); setCanvasShape('arrow'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'pen' && canvasShape === 'arrow' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Arrow"
                        >
                          ↗️
                        </button>
                        <button 
                          onClick={() => { setCanvasTool('pen'); setCanvasShape('star'); }}
                          className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'pen' && canvasShape === 'star' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                          title="Star"
                        >
                          ⭐
                        </button>
                      ...
                      <button 
                        onClick={() => { setCanvasTool('eraser'); setCanvasShape('free'); }}
                        className={`p-2.5 rounded-xl transition-all flex items-center gap-1 ${canvasTool === 'eraser' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                        title="Board Eraser"
                      >
                      ...
                      <div className="w-px h-6 bg-white/10 mx-1"></div>
                      <button 
                        onClick={() => { if(typeof (window as any).whiteboardUndo === 'function') (window as any).whiteboardUndo(); }}
                        className="p-2.5 rounded-xl transition-all text-slate-400 hover:text-white"
                        title="Undo"
                      >
                        ↩️
                      </button>
                      <button 
                        onClick={() => { if(typeof (window as any).whiteboardRedo === 'function') (window as any).whiteboardRedo(); }}
                        className="p-2.5 rounded-xl transition-all text-slate-400 hover:text-white"
                        title="Redo"
                      >
                        ↪️
                      ...
                      <button 
                        onClick={clearCanvas}
                        className="text-xs font-bold px-3 py-2 rounded-xl text-red-400 hover:bg-red-500/20 bg-red-500/10 transition-all border border-red-500/10 ml-2"
                      >
                        Clear Board
                      </button>
                      <button 
                        ...
                        className="text-xs font-bold px-3 py-2 rounded-xl text-indigo-400 hover:bg-indigo-500/20 bg-indigo-500/10 transition-all border border-indigo-500/10"
                      >
                        💾 Save PNG
                      </button>
                    </div>

                    {/* Dynamic brush ink colors */}
                    <div className="flex flex-wrap items-center gap-4">
                      
                      {/* Presets and Custom color picker container */}
                      <div className="flex items-center gap-2 bg-slate-950/50 p-2 rounded-2xl border border-white/5">
                        <div className="flex items-center gap-1.5">
                          {['#6366f1', '#10b981', '#3b82f6', '#f59e0b', '#ef4444', '#ec4899', '#ffffff', '#0f172a'].map(color => (
                            <button 
                              key={color}
                              onClick={() => {
                                setBrushColor(color);
                                setCanvasTool(canvasTool === 'eraser' ? 'pen' : canvasTool);
                              }}
                              className={`h-7 w-7 rounded-full border-2 transition-transform ${brushColor === color && canvasTool !== 'eraser' ? 'scale-125 border-white shadow-[0_0_15px_-3px_rgba(255,255,255,0.4)]' : 'border-black/50 hover:scale-110'}`}
                        ...
                        <div className="relative flex items-center justify-center p-1 cursor-pointer hover:bg-white/5 rounded-full" title="Custom color picker">
                          <input 
                            type="color" 
                            value={brushColor}
                            onChange={(e) => {
                              ...
                              if (!recentColors.includes(e.target.value)) {
                                setRecentColors(prev => [e.target.value, ...prev].slice(0, 5));
                              }
                            }}
                            className="opacity-0 w-8 h-8 absolute cursor-pointer inset-0 z-10"
                          />
                          <div 
                            ...
                            <div className="flex items-center gap-1">
                              {recentColors.map(color => (
                                <button 
                                  ...
                                  className={`h-6 w-6 rounded-full border-2 transition-transform ${brushColor === color && canvasTool !== 'eraser' ? 'scale-125 border-white' : 'border-black/50 hover:scale-110'}`}
                                  style={{ backgroundColor: color }}
                                />
                              ))}
                            </div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-400 font-semibold bg-slate-950/50 px-4 py-2 rounded-2xl border border-white/5">
                        <span>Size:</span>
                        <input 
                          type="range" 
                          min="2" 
                        ...
                        />
                        <span className="font-mono text-white min-w-[24px]">{brushSize}</span>
                      </div>
                    </div>
                  </div>

                  {/* Actual HTML Canvas element (Immersive) */}
                  <div className="flex-1 w-full relative cursor-crosshair min-h-[60vh] sm:h-full overflow-hidden">
                    <canvas 
                      ref={whiteboardRef}
                      ...
                      className="absolute inset-0 w-full h-full blackboard"
                    />
                  </div>

                  {/* Presenter / Live Feed Controls (Bottom HUD) */}
                  <div className="absolute bottom-6 left-6 z-10 flex items-center gap-4">
                    {effectiveRole === 'teacher' ? (
                      <div className="flex items-center gap-2 bg-slate-900/90 backdrop-blur-md p-2 rounded-2xl border border-white/10 shadow-lg">
                        <button 
                          onClick={handleTeacherBroadcast}
                          className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-4 py-2.5 rounded-xl uppercase tracking-wide transition-all shadow-md flex items-center gap-1.5"
                        >
                          📡 Broadcast Stream
                        </button>
                        {activeBroadcast && (
                          <button 
                            ...
                            className="bg-red-650 hover:bg-red-600 text-red-100 font-bold text-xs px-4 py-2.5 rounded-xl uppercase tracking-wide transition-all shadow-md"
                          >
                            Stop Stream
                          </button>
                        )}
                      ...
                      <div className="flex flex-col gap-2 bg-slate-900/90 backdrop-blur-md p-3 rounded-2xl border border-white/10 shadow-lg">
                         <button 
                          onClick={() => {
                            setStudentJoinedClass(!studentJoinedClass);
                            showNotification(studentJoinedClass ? 'Exited class feed.' : 'Subscribed to Teacher classroom smartboard stream!');
                          }}
                          className={`px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wide transition-all flex items-center gap-1.5 ${studentJoinedClass ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-slate-950 hover:bg-slate-800 text-indigo-400 border border-white/5'}`}
                        >
                          {studentJoinedClass ? '📡 Unsubscribe Live Class' : '📡 Subscribe to Class'}
                        </button>
                        {studentJoinedClass && (
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => sendStudentSignal('confused')} className="text-xl hover:scale-125 transition-transform" title="Confused">⚠️</button>
                            <button onClick={() => sendStudentSignal('understood')} className="text-xl hover:scale-125 transition-transform" title="Clear">✅</button>
                            <button onClick={() => sendStudentSignal('awesome')} className="text-xl hover:scale-125 transition-transform" title="Awesome">🚀</button>
                          </div>
                        )}
                      </div>
                    )}
                      ...
                      <span>Filters Match: <strong className="text-white font-mono">
                        {homeworkList.filter(hw => {
                          ...
                          if (effectiveRole === 'student') {
                            ...
                            const gradeMatch = currentUser?.assignedGrades?.includes(hw.classGrade);
                            const secMatch = hw.classSection === 'All Sections' || currentUser?.assignedSections?.includes(hw.classSection);
                            roleAllowed = gradeMatch && secMatch;
                          }
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                if (theFunMaterials.length > 1) {
                                  // remove duplicates by sorting (keeping the newest one first) then deleting the rest
                        ...
                        {(() => {
                          ...
                          if (materialsSearchQuery.trim()) {
                            ...
                            displayList = displayList.filter(m => 
                              ...
                              m.questionPaperYear?.includes(query)
                            );
                          }
                          ...
                          if (materialsSortBy === 'Newest') {
                            displayList.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
                      ...
                      <div className="bg-slate-900 border border-white/10 rounded-3xl max-w-3xl w-full p-6 space-y-5 shadow-2xl relative max-h-[92vh] overflow-y-auto text-left">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="space-y-3.5 bg-slate-950 p-4 rounded-2xl border border-white/5 flex flex-col justify-between">
                            ...
                            {/* DEBUG PANEL */}
                            <div className="mt-4 p-3 bg-red-950/40 border border-red-500/50 rounded-xl space-y-1.5 font-mono text-[9px] text-red-200 relative">
                              <h4 className="font-bold text-red-400 mb-2 border-b border-red-500/30 pb-1">🔧 DIAGNOSTIC DEBUG PANEL</h4>
                              
                              <div className="bg-black/50 p-2 rounded overflow-x-auto max-h-40">
                                <pre className="text-[8px] text-green-400">
                                  {JSON.stringify(previewMaterial, null, 2)}
                                </pre>
                              </div>
                              
                              <div className="mt-2 text-red-100 bg-red-900/50 p-2 rounded">
                                <p className="font-bold border-b border-red-500/30 mb-1">Live Network Test</p>
                                <button 
                                  onClick={async (e) => {
                                    const btn = e.currentTarget;
                                    const out = document.getElementById('debug-out-' + previewMaterial.id);
                                    if (!out || !previewMaterial.url) return;
                                    
                                    out.innerText = "Fetching...";
                                    try {
                                      const res = await fetch(previewMaterial.url, { method: 'GET' });
                                      ...
                                      out.innerText = `Fetch Failed: ${err.message || String(err)}\nLikely Cause: Missing Firebase Storage CORS Rules`;
                                    }
                                  }}
                                  className="bg-red-600 hover:bg-red-500 text-white px-2 py-1 rounded text-[10px] mt-1 inline-block"
                                >
                                  Test Fetch Network Error
                                </button>
                                <p id={`debug-out-${previewMaterial.id}`} className="mt-2 text-[10px] whitespace-pre-wrap font-mono"></p>
                              </div>
                            </div>
                    ...
                    <div className="smart-glass p-4 rounded-3xl h-[530px] flex flex-col justify-between shadow-lg relative border border-white/5">
                      ...
                      <div id="ai-dialog-scroll" className="flex-1 overflow-y-auto pr-2 my-3 space-y-3.5 bg-slate-950/40 p-4 rounded-2xl border border-white/5 shadow-inner">
                        <div className="text-center p-3 border border-white/5 bg-white/5 rounded-2xl">
                          <p className="text-[9px] text-amber-400 font-extrabold uppercase tracking-widest">Dialogue channel connected successfully</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-5 h-[650px] max-w-6xl mx-auto shadow-xl animate-fadeIn">
                  ...
                  <div className={`md:col-span-8 bg-slate-900/80 border border-white/5 rounded-3xl p-5 flex flex-col justify-between ${!showChatSidebarMobile ? 'flex' : 'hidden md:flex'}`}>
                    ...
                    <div id="chat-scroll-view" className="flex-1 overflow-y-auto my-4 space-y-3.5 pr-1 bg-slate-950/40 p-4 rounded-2xl border border-white/5 shadow-inner">
                      ...
                      {chats
                        .filter(msg => (msg.targetId || 'group-all') === activeChatTargetId)
                        .map((msg) => {
                          const isMe = msg.name === (currentUser?.name || '');
                          
                          // Check if there is an attached active material
                          const attachedMat = msg.sharedMaterialId ? materials.find(m => m.id === msg.sharedMaterialId) : null;

                          return (
                            <div
                              key={msg.id}
                              className={`flex flex-col gap-1 max-w-[85%] animate-fadeIn ${isMe ? 'ml-auto items-end' : 'mr-auto items-start'}`}
                            >
                              <div className={`p-3.5 rounded-2xl border text-left ${isMe ? 'bg-indigo-600/15 border-indigo-500/20 text-slate-200' : 'bg-slate-900 border-white/5 text-slate-300'}`}>
                                <span className="text-[9px] font-black text-indigo-400 block mb-1">
                                  {msg.name} ({msg.house || 'Faculty'}):
                                </span>
                                <p className="text-xs leading-relaxed break-words">{msg.message}</p>
                                
                                {/* Render Whatsapp style material attachment inside card */}
                                {attachedMat && (
                                  <div className="mt-2.5 p-3 rounded-xl bg-slate-950/80 border border-emerald-500/20 text-white space-y-2">
                                    <div className="flex items-center gap-2">
                                      <span className="text-lg">📁</span>
                                      <div className="min-w-0 flex-1">
                                        <p className="text-xs font-extrabold text-white truncate">{attachedMat.title}</p>
                                        <p className="text-[9px] text-slate-400 uppercase truncate">{attachedMat.subject} • {attachedMat.fileType || 'Doc'}</p>
                                      </div>
                                    </div>
                                    
                                    <div className="flex justify-between items-center text-[9px] text-slate-500 pt-1 border-t border-white/5">
                                      <span>📥 {attachedMat.downloads || 0} hits</span>
                                      {attachedMat.isVerified && <span className="text-emerald-400 font-bold">✓ Verified</span>}
                                    </div>
                                    
                                    <button
                                      onClick={() => {
                                        setPreviewMaterial(attachedMat);
                                        setActiveTab('materials');
                                      }}
                                      className="w-full mt-1 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold text-center transition-all inline-block hover:scale-102"
                                    >
                                      🔍 Open & study files
                                    </button>
                                  </div>
                                ...
                                <span className="text-[8px] text-slate-400 block text-right mt-1.5 font-mono">{msg.createdAt}</span>
                              </div>
                            </div>
                          );
                        })}

                      {chats.filter(msg => (msg.targetId || 'group-all') === activeChatTargetId).length === 0 && (
                        <div className="text-center py-10">
                          <p className="text-[11px] text-slate-500 italic">No messages shared yet. Start the conversation with your classmates!</p>
                        </div>
                      )}
                    </div>
            ...
            {isJarvisActive && (
              <>
                ...
                {/* Drawer */}
                <div className="fixed inset-y-0 right-0 w-full md:w-[600px] z-50 bg-slate-900 border-l border-white/10 shadow-2xl flex flex-col animate-fadeIn">
                  <StudentOSJarvis 
                    ...
                    onSuperAdminUnlocked={async () => {
                      ...
                      if (currentUser && currentUser.uid) {
                        try {
                          ...
                          showNotification('SYSTEM: Firestore authority elevated to Super Admin.');
                        } catch (e) {
                          console.error('Failed to elevate Firestore role:', e);
                        }
                      }
                    }}
                    currentUser={currentUser}
      ...
      {(DEV_MODE || isSuperAdmin) && currentUser && (
