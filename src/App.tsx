...
export default function App() {
      ...
      if (firebaseUser) {
        ...
        try {
          ...
          if (docExists) {
            ...
            // Automatically create a new student profile if document is missing
            console.log("USER DOC MISSING - AUTO-CREATING PROFILE");
            const newProfile: UserProfile = {
              uid: firebaseUser.uid,
              name: firebaseUser.displayName || (isWhitelisted ? 'Super Admin' : 'Student'),
            ...
            console.log("DASHBOARD READY");
            showNotification(`Profile auto-created. Welcome to StudentOS, ${newProfile.name}!`);
          }
        } catch (err: any) {
          ...
          showNotification(`⚠️ Role Load Failed. Please contact admin.`);
          
          // Do not log the user in with a fallback role.
          setCurrentUser(null);
          setFirebaseLoading(false);
  ...
  useEffect(() => {
    if (isDemoMode) {
      setTasks([
        { id: 'task-1', title: 'AP Physics Vectors Review', completed: false, category: 'Physics', priority: 'high', dueDate: 'Tomorrow', assignedBy: 'Prof. Elara' },
        { id: 'task-2', title: 'Complete organic chemistry synthesis homework', completed: true, category: 'Chemistry', priority: 'medium', dueDate: 'Friday', assignedBy: 'Dr. Ruby' },
        { id: 'task-3', title: 'BST Balancing Algorithms practice', completed: false, category: 'Computer Science', priority: 'low', dueDate: 'Next Monday' }
      ]);
      setVaultNotes([
        { id: 'note-1', title: 'Lecture Notes - Centripetal Mechanics', content: 'Centripetal force is directed toward the center of curvature. The magnitude is F = m v^2 / r. No work is performed globally in circular orbits because force vector is perpendicular to motion displacement.', category: 'Physics', lastEdited: Date.now() },
        { id: 'note-2', title: 'Chemistry Synthesis notes', content: 'Lewis acid acts as an electron pair acceptor, while Lewis base is an electron pair donor. Important to understand pH vs pKa relations for synthetic organic derivatives.', category: 'Chemistry', lastEdited: Date.now() }
      ]);
      setMaterials(INITIAL_MATERIALS);
      setFeedbackPosts(INITIAL_FEEDBACK);
      setAnnouncements(INITIAL_ANNOUNCEMENTS);
      setChats([
        { id: 'chat-1', senderName: 'Professor Elara', message: 'Welcome to the smart study group! Please feel free to ask formulas help.', timestamp: Date.now() - 3600000 },
        { id: 'chat-2', senderName: 'Naitik Kashyap', message: 'Thank you Professor! The formulas sheet has been super helpful.', timestamp: Date.now() - 1800000 }
      ]);
    ...
    // Clear all business data states before subscribing to prevent leakage/visual artifacts
    setTasks([]);
    setVaultNotes([]);
    setMaterials([]);
    setFeedbackPosts([]);
    ...
    const checkAllLoaded = (collectionName: string) => {
      loadedRefs.current[collectionName] = true;
      const required = ['tasks', 'notes', 'aiBuddyChats', 'materials', 'feedbacks', 'announcements', 'chats'];
      const allDone = required.every(key => loadedRefs.current[key]);
      if (allDone) {
        setDataLoading(false);
      }
    };
    ...
    // 7. Chats Sync DISABLED FOR SECURITY FIX
    // console.log("FIRESTORE CALL", "chats", "onSnapshot", uid, currentUser?.role);
    // const unsubscribeChats = onSnapshot(
    //   query(collection(db, 'chats'), where('ownerUid', '==', uid)),
    //   (snapshot) => {
    ...
    //   }
    // );
    checkAllLoaded('chats'); // Dummy check to let the rest load
                  ...
                  <div className={`md:col-span-8 bg-slate-900/80 border border-white/5 rounded-3xl p-5 flex flex-col justify-between ${!showChatSidebarMobile ? 'flex' : 'hidden md:flex'}`}>
                    ...
                    <div id="chat-scroll-view" className="flex-1 overflow-y-auto my-4 space-y-3.5 pr-1 bg-slate-950/40 p-4 rounded-2xl border border-white/5 shadow-inner">
                      ...
                      <div className="text-center py-10 bg-slate-900 border border-slate-800 rounded-xl">
                        <p className="text-[11px] text-slate-500 italic">Chat history temporarily unavailable.</p>
                      </div>
                    </div>
