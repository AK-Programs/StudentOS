...
export default function App() {
  ...
  const [regStep, setRegStep] = useState<'landing' | 'role' | 'profile' | 'submit' | 'super_admin_login'>('landing');
  const [superAdminEmail, setSuperAdminEmail] = useState('');
  const [superAdminPassword, setSuperAdminPassword] = useState('');
  const [isSuperAdminAuthenticating, setIsSuperAdminAuthenticating] = useState(false);
  ...
  const handleAINoteAction = async (action: string) => {
    ...
    try {
      ...
      if (resData.error) {
        showNotification(`⚠️ Engine fallback: ${resData.error}`);
        return;
      }
      ...
      console.error(err);
      showNotification('Could not establish connection with server-side AI.');
    } finally {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      showNotification('Google sign in cancelled or failed.');
      console.error(error);
    }
  };
  ...
  const handleLinkGoogle = async () => {
    try {
      ...
      showNotification('Google account linking cancelled or failed.');
      console.error(error);
    }
  };
  ...
  const handleTriggerMaterialAi = async (mat: MaterialResource, action: string) => {
    ...
    try {
      ...
      if (data.error) {
        showNotification(`⚠️ snags: ${data.error}`);
        setAiActionResultText(data.text || 'Error communicating with AI services.');
      } else {
        setAiActionResultText(data.text);
  ...
  const handleAskAIModel = async () => {
    ...
    try {
      ...
      const answer = parsedRes.text || 'I encountered an issue processing your lesson topic.';
        ...
        <div id="onboarding-gate-modal" className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 overflow-y-auto flex items-start sm:items-center justify-center p-4">
          <div className="smart-glass max-w-xl w-full p-6 sm:p-8 my-auto rounded-3xl border border-white/10 space-y-6 animate-fadeIn">
            ...
            <div className="space-y-6">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const firebaseUser = result.user;
                          if (firebaseUser) {
                            showNotification(`Signed in as ${firebaseUser.displayName}!`);
                          }
                        } catch (error) {
                          showNotification('Google sign in cancelled or failed.');
                          console.error(error);
                  ...
                  <div className="pt-4 border-t border-white/5 text-center space-y-3">
                    ...
                    <div className="pt-2 text-center">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setRegStep('super_admin_login');
                          setSuperAdminEmail('');
                          setSuperAdminPassword('');
                        }}
                        className="text-xs font-bold text-indigo-400 hover:text-indigo-300 hover:underline transition-all cursor-pointer"
                      >
                        Super Admin Login
                      </button>
                    </div>
              ...
              {regStep === 'super_admin_login' && (
                <form
                  id="reg-step-super-admin-login"
                  onSubmit={async (e) => {
                    e.preventDefault();
                    ...
                    setIsSuperAdminAuthenticating(true);
                    try {
                      ...
                      try {
                        ...
                        const q = query(collection(db, 'users'), where('email', '==', superAdminEmail.toLowerCase()));
                        ...
                        const currentRole = docExists ? existingProfile.role : 'null';
                        const currentStatus = docExists ? existingProfile.accountStatus : 'null';
                        
                        console.log('==================================================');
                        console.log('SUPER ADMIN BYPASS LOGIN SYNC');
                        console.log(`Current User UID: ${targetUid}`);
                        console.log(`Current Email: ${superAdminEmail}`);
                        console.log(`Current Firestore Role: ${currentRole}`);
                        console.log(`Current Firestore Path: users/${targetUid}`);
                        console.log(`Operation Type: ${docExists ? 'setDoc (merge: true)' : 'setDoc (create)'}`);
                        console.log(`Document exists? ${docExists ? 'YES' : 'NO'}`);
                        console.log(`Current role? ${currentRole}`);
                        console.log(`Current accountStatus? ${currentStatus}`);
                        console.log('==================================================');
                        ...
                        if (docExists) {
                          // Update safely without creating duplicate documents or overwriting other fields
                          profile = { ...existingProfile, role: 'super_admin', accountStatus: 'approved' };
                          await setDoc(doc(db, 'users', targetUid), { role: 'super_admin', accountStatus: 'approved' }, { merge: true });
                          console.log(`Successfully merged super_admin role into existing document users/${targetUid}`);
                        } else {
                          // Create new profile safely
                          await setDoc(doc(db, 'users', targetUid), profile);
                          console.log(`Successfully created new super_admin document users/${targetUid}`);
                        }
                      } catch (dbErr) {
                        console.error('Error synchronizing super admin with firestore:', dbErr);
                      }
                      ...
                      const updatedRecent = [profile, ...recentAccounts.filter((a: any) => a.email !== profile.email)].slice(0, 5);
                      setRecentAccounts(updatedRecent);
                      localStorage.setItem('s_os_recent_accounts', JSON.stringify(updatedRecent));
                      
                      setCurrentUser(profile);
                      setActiveTab('admin');
                      
                      showNotification('Welcome Super Administrator! Orion Control Center Activated.');
                    } catch (err: any) {
                      showNotification(`❌ Error: ${err.message}`);
                    } finally {
                      setIsSuperAdminAuthenticating(false);
                    }
                  }}
                  className="space-y-5 animate-fadeIn"
                >
                  <div className="space-y-1 text-center">
                    <span className="text-[10px] uppercase font-black tracking-widest text-indigo-400">🛡️ Security Clearance Node</span>
                    <h2 className="text-xl font-bold text-white">Super Admin Credentials</h2>
                    <p className="text-xs text-slate-400">Provide whitelisted administrator credentials to gain bypass access.</p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        placeholder="naitik.kashyap0015@gmail.com"
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-400 block">System Password</label>
                      <input
                        type="password"
                        required
                        ...
                        placeholder="••••••••"
                        className="w-full text-sm px-4 py-3 rounded-xl bg-slate-900 border border-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-white"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-white/5">
                    ...
                    <button
                      type="submit"
                      disabled={isSuperAdminAuthenticating}
                      className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-extrabold uppercase tracking-wider transition-all disabled:opacity-50 cursor-pointer"
                    >
                      {isSuperAdminAuthenticating ? 'Authenticating...' : 'Bypass Login →'}
                    </button>
                  </div>
                </form>
              )}
