...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (code && !hasExchanged) {
      ...
      console.log("[SUPABASE OAUTH] Hash params found on mount. Setting loading states. Leaving hash intact for Supabase to parse.");
      setFirebaseLoading(true);
      setDataLoading(true);
    }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      debugObj.events.push({ event, timestamp: new Date().toISOString() });
  ...
  const debugState = (window as any).__STUDENTOS_AUTH_DEBUG__;
  return (
