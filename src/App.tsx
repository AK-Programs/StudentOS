...
import { saveSupabaseMaterial } from './lib/supabaseResources';
...
import { query, collection, where, onSnapshot, setDoc, doc, updateDoc, deleteDoc } from 'firebase/firestore';
...
export default function App() {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        try {
          ...
          console.log("LOADING SUPABASE USER PROFILE");
          ...
          try {
            profile = await getSupabaseUserProfile(firebaseUser.uid);
          } catch (sbErr: any) {
            console.error('[Supabase Auth] Failed to fetch user profile:', sbErr);
            if (sbErr.code === 'PGRST205') {
              isTableMissing = true;
              console.warn('[Supabase Auth] Table "users" does not exist yet. Running with local fallback.');
            } else {
              throw sbErr;
            }
          }
          ...
          if (profile) {
            console.log("ROLE_LOADED from Supabase");
            ...
            const newProfile: UserProfile = {
              ...
              email: firebaseUser.email || `${firebaseUser.uid}@temp.user.com`,
            ...
            if (isTableMissing) {
              profile = newProfile;
              showNotification('⚠️ Supabase "users" table not found. Please run the SQL migration script! Running in demo/offline mode.');
            } else {
              try {
          ...
          console.log("Error code:", err.code || "unknown_code");
  ...
  // Tasks Sync On-Demand (Lazy-Loaded)
  useEffect(() => {
    if (isDemoMode || !auth.currentUser || !currentUser || (activeTab !== 'tasks' && activeTab !== 'dashboard')) return;
  ...
  }, [auth.currentUser, activeTab, isDemoMode]);

  // Personal Notes Sync On-Demand (Lazy-Loaded)
  useEffect(() => {
    if (isDemoMode || !auth.currentUser || !currentUser || activeTab !== 'notes') return;
    const uid = auth.currentUser.uid;
    ...
    const unsubscribeNotes = onSnapshot(
      ...
      (error) => {
        console.error("Firestore error on notes snapshot:", error);
  ...
  useEffect(() => {
    if (isDemoMode || !auth.currentUser || !currentUser || activeTab !== 'ai_teacher') return;
    const uid = auth.currentUser.uid;
  ...
  }, [auth.currentUser, activeTab, isDemoMode, currentUser]);

  // Educational Materials Hub Sync On-Demand (Lazy-Loaded)
  useEffect(() => {
    if (isDemoMode || !auth.currentUser || !currentUser || activeTab !== 'materials') return;
    const uid = auth.currentUser.uid;
  ...
  // Feedback Sync On-Demand (Lazy-Loaded)
  useEffect(() => {
    if (isDemoMode || !auth.currentUser || !currentUser || activeTab !== 'feedback') return;
    const uid = auth.currentUser.uid;
  ...
  // Announcements Sync On-Demand (Lazy-Loaded)
  useEffect(() => {
    if (isDemoMode || !auth.currentUser || !currentUser || activeTab !== 'dashboard') return;
    const uid = auth.currentUser.uid;
  ...
  useEffect(() => {
    if (isDemoMode || !auth.currentUser || !currentUser || activeTab !== 'peer_chat') return;
    console.log("[SUPABASE-CHAT] Syncing messages and custom rooms...");
    getPeerMessages().then(list => {
      if (list) {
        console.log(`[SUPABASE-CHAT] Component received MESSAGES_COUNT: ${list.length}, CURRENT_USER: ${auth.currentUser?.uid}, CURRENT_ROOM_ID/CHAT_ROOM_ID: ${activeChatTargetId}`);
        setChats(list);
        setTimeout(() => {
    ...
    getChatRooms().then(roomsList => {
      if (roomsList) {
        setChatRooms(prev => {
          const defaults = prev.filter(r => r.id.startsWith('group-') || r.id.startsWith('friend-') || r.id.startsWith('channel-'));
          const custom = roomsList.filter(r => !defaults.some(d => d.id === r.id));
