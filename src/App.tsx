      ...
      if (firebaseUser) {
        ...
        const fetchPromise = new Promise(async (resolve, reject) => {
          fetchTimeout = setTimeout(() => {
            reject(new Error("Timeout: Firestore did not respond within 10 seconds."));
          }, 10000);
          
          try {
            console.log("ENVIRONMENT DIAGNOSTICS:");
            console.log("VITE_FIREBASE_PROJECT_ID loaded:", !!import.meta.env.VITE_FIREBASE_PROJECT_ID);
            console.log("VITE_FIREBASE_AUTH_DOMAIN loaded:", !!import.meta.env.VITE_FIREBASE_AUTH_DOMAIN);
            console.log("VITE_FIREBASE_FIRESTORE_DATABASE_ID loaded:", !!import.meta.env.VITE_FIREBASE_FIRESTORE_DATABASE_ID);
            ...
            clearTimeout(fetchTimeout);
            console.error("FIRESTORE READ ERROR DETAILS:", err);
        ...
        try {
          const userDoc: any = await fetchPromise;
          ...
          if (err.code === 'permission-denied') {
            errorMessage += ' — PLEASE DEPLOY FIRESTORE RULES TO YOUR CUSTOM FIREBASE PROJECT!';
          }

          setAuthError({
            code: err.code || 'firestore/read-error',
            message: errorMessage,
            hostname: window.location.hostname
          });
          // Show alert and stop loading so error is visible
          showNotification(`❌ Error loading user: ${errorMessage}`);
          setFirebaseLoading(false); // Stop loading screen
          setShowStartup(false); // Force startup screen off immediately
          setCurrentUser(null); // Force back to modal
        }
      } else {
