...
export default function App() {
      ...
      if (supabaseUser) {
        ...
        try {
          ...
          if (profile) {
            ...
            setRegEmail(supabaseUser.email || '');
            setRegName(supabaseUser.user_metadata?.full_name || supabaseUser.user_metadata?.name || '');
            setRegPhoto(supabaseUser.user_metadata?.avatar_url || supabaseUser.user_metadata?.picture || '');
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (localNotes && localNotes !== "undefined") {
        setVaultNotes(JSON.parse(localNotes));
      } else {
        setVaultNotes([]);
      }
    } catch (err) {
      console.error('[Sync] Failed to parse local notes:', err);
      setVaultNotes([]);
    }
  }, [currentUser]);
