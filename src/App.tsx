...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (supabaseUser) {
        console.log("AUTH_SUCCESS via Supabase");
        ...
        try {
          ...
          try {
            profile = await getSupabaseUserProfile(supabaseUser.id);
          } catch (sbErr: any) {
            console.error('[Supabase Auth] Failed to fetch user profile:', sbErr);
            if (sbErr.code === 'PGRST205' || sbErr.status === 404) {
              isTableMissing = true;
          ...
          console.error('[Supabase Auth] Error reading user record:', err);
  ...
  // Tasks Sync
  useEffect(() => {
    if (!currentUser || (activeTab !== 'tasks' && activeTab !== 'dashboard')) return;
  ...
  // Personal Notes Sync
  useEffect(() => {
    if (!currentUser || activeTab !== 'notes') return;
    const uid = currentUser.uid;
    const localNotes = localStorage.getItem(`s_os_notes_${uid}`);
    if (localNotes) {
      setVaultNotes(JSON.parse(localNotes));
    } else {
      setVaultNotes([]);
    }
  }, [currentUser, activeTab]);
