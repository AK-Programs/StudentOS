...
export default function HouseChampionship({ currentUser, effectiveRole, showNotification }: any) {
  ...
  const handleSaveLeaders = async (houseId: string) => {
    try {
      ...
      await setDoc(doc(db, 'houseAnalytics', houseId), {
        captainName,
        captainGrade,
        viceCaptainName,
        viceCaptainGrade
      }, { merge: true });
      showNotification(`${houseId} Leaders updated successfully.`);
             ...
             <div className="mb-4 bg-slate-950/60 p-3 rounded-2xl text-left border border-white/5 flex-grow">
               ...
               {editingHouse === h.id ? (
                 <div className="space-y-2">
                   <div className="space-y-1">
                     <label className="text-[8px] text-slate-400 uppercase">Captain Name & Grade</label>
                     <input type="text" placeholder="Name" value={captainName} onChange={e=>setCaptainName(e.target.value)} className="w-full text-xs px-2 py-1 bg-slate-900 border border-white/10 rounded text-white" />
                     <input type="text" placeholder="Grade/Section" value={captainGrade} onChange={e=>setCaptainGrade(e.target.value)} className="w-full text-xs px-2 py-1 bg-slate-900 border border-white/10 rounded text-white" />
                   </div>
                   <div className="space-y-1">
                     <label className="text-[8px] text-slate-400 uppercase">Vice Captain Name & Grade</label>
                     <input type="text" placeholder="Name" value={viceCaptainName} onChange={e=>setViceCaptainName(e.target.value)} className="w-full text-xs px-2 py-1 bg-slate-900 border border-white/10 rounded text-white" />
                     <input type="text" placeholder="Grade/Section" value={viceCaptainGrade} onChange={e=>setViceCaptainGrade(e.target.value)} className="w-full text-xs px-2 py-1 bg-slate-900 border border-white/10 rounded text-white" />
                   </div>
                   <div className="flex gap-2 pt-2 text-[10px]">
                     <button onClick={() => handleSaveLeaders(h.id)} className="bg-indigo-500 text-white px-2 py-1 rounded flex-1">Save</button>
                     <button onClick={() => setEditingHouse(null)} className="bg-slate-700 text-white px-2 py-1 rounded flex-1">Cancel</button>
                   </div>
                 </div>
               ) : (
                 <div className="space-y-3">
                   ...
                   {canEdit && (
                     <button onClick={() => handleEditClick(h)} className="w-full mt-2 py-1.5 border border-white/10 hover:bg-white/10 rounded-lg text-[10px] font-bold text-slate-300 transition-colors">
                       Manage Leaders
                     </button>
                   )}
                 </div>
