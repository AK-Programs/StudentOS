...
interface Shape { type: 'rect' | 'circle' | 'triangle' | 'line'; id: string; x: number; y: number; width?: number; height?: number; radius?: number; fill?: string; stroke?: string; }
interface LineData { id: string; tool: string; points: number[]; color: string; brushSize: number; }
...
export function Whiteboard2({ onClose, initialPrompt, currentUser }: { onClose?: () => void; initialPrompt?: string; currentUser?: any }) {
  ...
  const handleErase = (id: string, type: 'shapes'|'lines'|'stickies') => {
    if (tool === 'eraser') {
