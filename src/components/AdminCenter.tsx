...
import { db, sendNotificationToUsers } from '../firebase';
...
export default function AdminCenter({ currentUser, showNotification, profileTab }: { currentUser: UserProfile; showNotification: (msg: string) => void, profileTab?: string }) {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      snap.forEach(d => {
        ...
        // Make sure we carry the document id if uid isn't stored inside
        if (!data.uid) {
          data.uid = d.id;
        }
        list.push(data);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const docId = editingUser.uid || editingUser.email;
      await setDoc(doc(db, 'users', docId), editingUser);
  ...
  const handleApprove = async (user: UserProfile) => {
    ...
    // 1. Fetch current user's actual stored role in Firestore (to bypass UI-only / simulation bypasses)
    let firestoreRole = 'unknown';
    if (currentUser && currentUser.uid) {
      try {
        const selfDoc = await getDoc(doc(db, 'users', currentUser.uid));
        if (selfDoc.exists()) {
          firestoreRole = (selfDoc.data() as UserProfile).role || 'student';
        }
      } catch (err) {
        console.error('Error verifying Firestore role:', err);
      }
    }
    ...
    console.log(`Current Role (Firestore Stored): ${firestoreRole}`);
    console.log(`Current Effective Role: ${currentUser.role || 'unknown'}`);
    console.log('=================================================');

    // Guard: Coordinator and Admin approvals must require actual Firestore 'super_admin' role
    ...
    if (user.requestedRole === 'coordinator' || user.requestedRole === 'admin') {
      if (firestoreRole === 'teacher') {
        showNotification(`❌ Approval Denied: Your verified Firestore role is "teacher". Super Admin clearance is required.`);
        return;
      }
      if (!isRealSuperAdmin) {
        showNotification(`❌ Approval Denied: Verified Firestore role "super_admin" is required to approve ${user.requestedRole}s. Current Stored Role: ${firestoreRole}`);
        return;
      }
    }
  ...
  const generateDemoData = async () => {
    ...
    try {
      ...
      for (const u of demousers) {
        await setDoc(doc(db, 'users', u.uid || u.email), u);
      }
