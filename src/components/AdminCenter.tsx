            ...
            {editingUser.role === 'teacher' && (
              <>
                <div>
                  ...
                  <input type="text" value={(editingUser.assignedClasses || []).join(', ')} onChange={e => setEditingUser({...editingUser, assignedClasses: e.target.value.split(',').map(s => s.trim()).filter(Boolean)})} className="w-full px-3 py-2 bg-slate-950 border border-white/10 rounded-xl text-white text-sm" placeholder="e.g. 9A, 9B, 10A" />
                  <p className="text-[10px] text-slate-500 mt-1">Format: Grade + Section (e.g. 9A, 10B)</p>
                </div>
              </>
            )}
