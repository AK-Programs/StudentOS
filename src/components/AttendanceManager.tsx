...
export default function AttendanceManager({ currentUser, effectiveRole }: { currentUser: UserProfile; effectiveRole: UserRole | undefined }) {
    ...
    try {
      ...
      if (targetUser) {
        sendNotificationToUsers({
          ...
          message: `${targetUser.name} was marked "${status.toUpperCase()}" on ${date}.`,
          type: 'attendance'
  ...
  const applyLeave = async () => {
    ...
    try {
      ...
      // Dispatch alert
      sendNotificationToUsers({
        title: '📂 New Leave Requested',
        message: `${currentUser.name} applied for leave on ${leaveDates}. Reason: "${leaveReason}".`,
        type: 'leave'
      }).catch(e => console.error(e));
    ...
    try {
      ...
      if (req) {
        sendNotificationToUsers({
          ...
          message: `Your leave request for ${req.dates} has been ${status} by ${currentUser.name}.`,
          type: 'leave'
        }).catch(e => console.error(e));
      }
