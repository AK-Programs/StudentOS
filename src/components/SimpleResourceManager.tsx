...
export default function SimpleResourceManager({ 
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      // Query collection directly to avoid index errors on unindexed fields
      const q = query(collection(db, 'schoolResources'));
      ...
      handleFirestoreError(err, OperationType.LIST, 'schoolResources');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'schoolResources', id), dataToSave, { merge: true });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'schoolResources', id));
          ...
          {items.map(item => (
            <div key={item.id} className="bg-slate-900 p-5 rounded-2xl border border-white/5 flex flex-col md:flex-row gap-4 justify-between group hover:border-white/10 transition-all">
              ...
              <div className="flex md:flex-col gap-2 items-end justify-center min-w-[120px]">
                {item.fileData && type !== 'gallery' && (
                  <a href={item.fileData} download={item.fileName} className="w-full justify-center px-3 py-2 bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 rounded-lg text-xs font-bold flex items-center gap-2 transition-all">
