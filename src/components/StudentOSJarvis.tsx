...
import { 
  collection, getDocs, doc, setDoc, addDoc, query, 
...
export const StudentOSJarvis: React.FC<StudentOSJarvisProps> = ({
  ...
  // Sync data from Firestore & Seed initial indicators if empty
  useEffect(() => {
    if (!currentUser) return;

    if (!auth.currentUser) {
      // Mock mode fallback: populate state immediately without querying firestore!
      setReports([
        {
          id: 'rep-aditya',
          name: 'Aditya Sen',
          ...
          recentActivity: ['Completed Physics Mock Exam', 'Shared Chem Formulas Checklist', 'Won Algebra Live League']
        },
        {
          id: 'rep-chloe',
          name: 'Chloe Lin',
          ...
          communityContributions: 38,
          recentActivity: ['Submitted CS JavaScript Project', 'Earned Emerald House Star', 'Uploaded Cell Bio PPTX']
        },
        {
          id: 'rep-marcus',
          name: 'Marcus Vance',
          ...
          recentActivity: ['Achieved 100% in Geometry Quiz', 'Joined peer chat channel']
        },
        {
          id: 'rep-kiara',
          name: 'Kiara Jha',
          ...
          communityContributions: 30,
          recentActivity: ['Wrote detailed World War History summary', 'Analyzed Motion equations PDF']
        }
      ]);
      ...
      setSections([
        { id: 'Astra', totalStudents: 32, studyActivity: 45, quizScores: 84, materialContributions: 54 },
        { id: 'Elera', totalStudents: 28, studyActivity: 38, quizScores: 79, materialContributions: 41 },
        { id: 'Solara', totalStudents: 30, studyActivity: 48, quizScores: 88, materialContributions: 62 },
        { id: 'Vega', totalStudents: 31, studyActivity: 42, quizScores: 82, materialContributions: 49 }
      ]);
    ...
    // 1. Fetch / listen to student reports
    console.log("FIRESTORE CALL", "studentReports", "onSnapshot", auth.currentUser?.uid, currentUser?.role);
    const unsubReports = onSnapshot(collection(db, 'studentReports'), (snap) => {
      if (snap.empty) {
        seedInitialReports();
      } else {
        const list: StudentReport[] = [];
        snap.forEach(doc => list.push({ id: doc.id, ...doc.data() } as StudentReport));
        setReports(list);
      }
    }, (err) => {
      alert(
      ...
      handleFirestoreError(err, OperationType.LIST, 'studentReports');
    });

    // 2. Fetch / listen to house standings
    console.log("FIRESTORE CALL", "houseAnalytics", "onSnapshot", auth.currentUser?.uid, currentUser?.role);
    const unsubHouses = onSnapshot(collection(db, 'houseAnalytics'), (snap) => {
      if (snap.empty) {
        seedInitialHouses();
      } else {
        const list: HouseAnalytics[] = [];
        snap.forEach(doc => list.push({ id: doc.id, ...doc.data() } as HouseAnalytics));
        // Sort by points
      ...
      alert(
        "Firestore Error\n" +
        "Collection: houseAnalytics\n" +
        "Operation: onSnapshot\n" +
        "Message: " + err.message
      );
      handleFirestoreError(err, OperationType.LIST, 'houseAnalytics');
    });

    // 3. Fetch / listen to section analytics
    console.log("FIRESTORE CALL", "sectionAnalytics", "onSnapshot", auth.currentUser?.uid, currentUser?.role);
    const unsubSections = onSnapshot(collection(db, 'sectionAnalytics'), (snap) => {
      if (snap.empty) {
        seedInitialSections();
      } else {
        const list: SectionAnalytics[] = [];
        snap.forEach(doc => list.push({ id: doc.id, ...doc.data() } as SectionAnalytics));
        setSections(list);
      }
    }, (err) => {
      alert(
        "Firestore Error\n" +
        "Collection: sectionAnalytics\n" +
        "Operation: onSnapshot\n" +
        "Message: " + err.message
      );
      handleFirestoreError(err, OperationType.LIST, 'sectionAnalytics');
    });

    // 4. Fetch / listen to history items
    let unsubHistory: any = () => {};
    let unsubCommands: any = () => {};
    if (auth.currentUser?.uid) {
      console.log("FIRESTORE CALL", "orionChats", "onSnapshot", auth.currentUser?.uid, currentUser?.role);
      unsubHistory = onSnapshot(
        query(collection(db, 'orionChats'), where('userId', '==', auth.currentUser.uid), orderBy('timestamp', 'desc'), limit(15)), 
        (snap) => {
          const list: JarvisHistoryItem[] = [];
          snap.forEach(doc => list.push({ id: doc.id, ...doc.data() } as JarvisHistoryItem));
          setHistoryItems(list);
        }, (err) => {
          alert(
            "Firestore Error\n" +
            "Collection: orionChats\n" +
            "Operation: onSnapshot\n" +
            "Message: " + err.message
          );
          console.warn('History subscription restricted temporarily', err);
        }
      );
      // 5. Fetch / listen to commands log (per user isolation)
      console.log("FIRESTORE CALL", `users/${auth.currentUser.uid}/teacherCommands`, "onSnapshot", auth.currentUser?.uid, currentUser?.role);
      unsubCommands = onSnapshot(
        query(collection(db, `users/${auth.currentUser.uid}/teacherCommands`), orderBy('recognizedAt', 'desc'), limit(15)),
        (snap) => {
          const list: TeacherCommand[] = [];
          snap.forEach(doc => list.push({ id: doc.id, ...doc.data() } as TeacherCommand));
          setCommandAudit(list);
        }, (err) => {
          alert(
            "Firestore Error\n" +
            "Collection: teacherCommands\n" +
            "Operation: onSnapshot\n" +
            "Message: " + err.message
          );
          console.warn('Audit subscription restricted temporarily', err);
        }
      );
    }
  ...
  const seedInitialReports = async () => {
    ...
    for (const st of mockReports) {
      await setDoc(doc(db, 'studentReports', st.id), st);
    }
  };

  const seedInitialHouses = async () => {
    const mockHouses: HouseAnalytics[] = [
      { id: 'Ruby', points: 1450, materialsShared: 38, quizzesWon: 27, participation: 91 },
      { id: 'Emerald', points: 1580, materialsShared: 42, quizzesWon: 30, participation: 94 },
      { id: 'Sapphire', points: 1320, materialsShared: 29, quizzesWon: 22, participation: 87 },
      { id: 'Topaz', points: 1410, materialsShared: 35, quizzesWon: 25, participation: 89 }
    ...
    for (const h of mockHouses) {
      await setDoc(doc(db, 'houseAnalytics', h.id), h);
    }
  };

  const seedInitialSections = async () => {
    const mockSections: SectionAnalytics[] = [
      { id: 'Astra', totalStudents: 32, studyActivity: 45, quizScores: 84, materialContributions: 54 },
      { id: 'Elera', totalStudents: 28, studyActivity: 38, quizScores: 79, materialContributions: 41 },
      { id: 'Solara', totalStudents: 30, studyActivity: 48, quizScores: 88, materialContributions: 62 },
      { id: 'Vega', totalStudents: 31, studyActivity: 42, quizScores: 82, materialContributions: 49 }
    ...
    for (const s of mockSections) {
      await setDoc(doc(db, 'sectionAnalytics', s.id), s);
    }
  };
  ...
  const executeVoiceCommand = async (textToParse: string) => {
    ...
    try {
      if (auth.currentUser?.uid) {
        ...
        await setDoc(doc(db, 'orionChats', histId), {
          id: histId,
          ...
          actionExecuted: actionTriggered
        });

        const auditId = `audit-${Date.now()}`;
        await setDoc(doc(db, `users/${auth.currentUser.uid}/teacherCommands`, auditId), {
          id: auditId,
