...
export const StudentOSJarvis: React.FC<StudentOSJarvisProps> = ({
  ...
  const handleGenerateNotes = async (topic: string) => {
    ...
    try {
      ...
      showNotification(`✓ Orion has drafted your "${topic}" packet. Click "Save to Lecture Notes" below to save to your Vault!`);
    } catch (err) {
                ...
                {searchQuery.trim().length > 2 && (
                  <div className="flex flex-wrap gap-3 pt-3">
                    <button 
                      ...
                      className="px-4 py-2 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 hover:text-indigo-200 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 border border-indigo-500/20"
                    >
                      <Sparkles className="w-4 h-4" /> Generate Vault Notes
                    </button>
                    <button 
                      onClick={() => handleGenerateLessonPlan(searchQuery)}
                      className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 hover:text-emerald-200 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 border border-emerald-500/20"
                    >
                      <Layers className="w-4 h-4" /> Generate Study Plan Slot
                    </button>
                  </div>
                )}
              </div>
