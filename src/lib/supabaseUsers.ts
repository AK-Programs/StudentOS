...
export async function getSupabaseUserProfile(uid: string, email?: string): Promise<UserProfile | null> {
  ...
  if (!data && email) {
    console.log('[SUPABASE-USERS] Profile not found by uid, falling back to email search for:', email);
    const { data: emailTry, error: emailError } = await supabase
      .from('users')
      .select('*')
      .eq('email', email)
      .maybeSingle();
    ...
    if (emailError) {
       console.error('[SUPABASE-USERS] Error searching by email:', emailError);
    } else if (emailTry) {
       console.log('[SUPABASE-USERS] Profile found by email, linking to new uid:', uid);
       data = emailTry;
       // Link the new uid to the existing profile
       await supabase
         .from('users')
         .update({ id: uid })
         .eq('email', email);
...
export async function saveSupabaseUserProfile(profile: UserProfile): Promise<UserProfile> {
  ...
  // Check if user exists by id, firebase_uid, OR email
  ...
  let result;
  if (finalExisting) {
    console.log('[SUPABASE-USERS] Updating existing row id:', finalExisting.id);
    const { data, error } = await supabase
      .from('users')
      .update(dbUser)
      .eq('id', finalExisting.id)
      .select()
      .single();
