    ...
    if (existing) {
      ...
      try {
        ...
        const isUniqueViolation = 
          ...
          String(insertErr?.message || '').toLowerCase().includes('already exists');
          
        if (isUniqueViolation) {
          console.warn('[SUPABASE-USERS] Unique violation on insert, converting to update...');
          let foundRow: any = null;
          if (isValidUUID(profile.uid)) {
            try {
              const { data } = await supabase.from('users').select('*').eq('id', profile.uid).maybeSingle();
              if (data) foundRow = data;
            } catch (e) {}
          }
          if (!foundRow) {
            try {
              const { data } = await supabase.from('users').select('*').eq('firebase_uid', profile.uid).maybeSingle();
              if (data) foundRow = data;
            } catch (e) {}
          }
          if (!foundRow && profile.email) {
            try {
              const { data } = await supabase.from('users').select('*').eq('email', profile.email.toLowerCase()).maybeSingle();
              if (data) foundRow = data;
            } catch (e) {}
          }
          ...
          if (foundRow) {
            existing = foundRow;
            console.log('[SUPABASE-USERS] Resolved existing row during violation retry:', existing.id);
            let updateQuery;
            if (existing.id) {
              updateQuery = supabase.from('users').update(pl).eq('id', existing.id).select().single();
            } else {
